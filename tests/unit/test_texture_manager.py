...
def test_memory_limit():
    ...
    try:
        manager = TextureManager(hw.ctx, max_textures=2)
        manager.create_texture("1", (10, 10), 1)
        manager.create_texture("2", (10, 10), 1)
...
def test_create_from_image_mocked(mock_exists, mock_pil):
    hw = OpenGLContext(1280, 720, headless=True)
    hw.make_current()
    if not hw.ctx:
        hw.terminate()

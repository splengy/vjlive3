...
def plugin():
    ...
    return DepthTemporalEchoPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == "Depth Temporal Echo"
    assert "depth_in" in m["inputs"]
    pnames = [p["name"] for p in m["parameters"]]
    assert "ghost_opacity" in pnames; assert "far_delay" in pnames
    assert len(pnames) == 6


@patch('vjlive3.plugins.depth_temporal_echo.gl', mock_gl)
...
@patch('vjlive3.plugins.depth_temporal_echo.gl', mock_gl)
@patch('vjlive3.plugins.depth_temporal_echo.hasattr')
...
def test_process_frame(plugin, context):
    plugin.initialize(context)
    params = {"ghost_opacity": 0.7, "color_decay": 0.3, "blend_mode": 0.0,
              "near_delay": 0.0, "far_delay": 0.8, "edge_bleed": 0.4}

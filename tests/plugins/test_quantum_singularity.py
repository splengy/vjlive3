"""Tests for P6-QC14: QuantumSingularityPlugin."""
import pytest
...
from vjlive3.plugins.quantum_singularity import QuantumSingularityPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return QuantumSingularityPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Quantum Singularity'
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.quantum_singularity.gl', _mock_gl)
...
@patch('vjlive3.plugins.quantum_singularity.gl', _mock_gl)
@patch('vjlive3.plugins.quantum_singularity.hasattr')

"""Tests for P7-VE43: ProjectionMappingPlugin."""
import pytest
...
from vjlive3.plugins.projection_mapping2 import ProjectionMappingPlugin, METADATA
...
def plugin():
    ...
    _gl.glCreateProgram.return_value=99; _gl.glGenVertexArrays.return_value=44
    _gl.glGenTextures.return_value=55; _gl.glGenFramebuffers.return_value=51
    return ProjectionMappingPlugin()
@pytest.fixture
def context():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Projection Mapping2'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.projection_mapping2.gl', _gl)
...
@patch('vjlive3.plugins.projection_mapping2.gl', _gl)
@patch('vjlive3.plugins.projection_mapping2.hasattr')

...
from vjlive3.plugins.depth_distortion import DepthDistortionPlugin, METADATA
...
def test_plugin_metadata(plugin):
    ...
    assert meta["name"] == "DepthDistortion"
    assert meta["plugin_type"] == "depth_effect"
    assert "distortionType" in [p["name"] for p in meta["parameters"]]

@patch('vjlive3.plugins.depth_distortion.gl', mock_gl)
...
@patch('vjlive3.plugins.depth_distortion.gl', mock_gl)
@patch('vjlive3.plugins.depth_distortion.hasattr')
...
def test_process_frame_standard_execution(plugin, context):
    ...
    params = {
        ...
        "distortionType": 2, # Bulge
        "depthThreshold": 2.5,
        "distortionRadius": 0.8
    }
    ...
    mock_gl.glUniform1f.assert_any_call(mock_gl.glGetUniformLocation(99, "distortion_strength"), 0.5)
    mock_gl.glUniform1i.assert_any_call(mock_gl.glGetUniformLocation(99, "distortion_type"), 2)
    mock_gl.glUniform1f.assert_any_call(mock_gl.glGetUniformLocation(99, "depth_threshold"), 2.5)
    mock_gl.glUniform1f.assert_any_call(mock_gl.glGetUniformLocation(99, "distortion_radius"), 0.8)

"""Tests for P5-DM16: Datamosh3DPlugin."""
import pytest
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Datamosh 3D'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.datamosh_3d.gl', _mock_gl)
...
@patch('vjlive3.plugins.datamosh_3d.gl', _mock_gl)
@patch('vjlive3.plugins.datamosh_3d.hasattr')

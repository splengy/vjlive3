"""Tests for P5-DM08: CottonCandyDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.cotton_candy_datamosh import CottonCandyDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return CottonCandyDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Cotton Candy Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.cotton_candy_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.cotton_candy_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.cotton_candy_datamosh.hasattr')

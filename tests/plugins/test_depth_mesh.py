...
import vjlive3.plugins.depth_mesh as dm
from vjlive3.plugins.depth_mesh import DepthMeshPlugin, METADATA
from vjlive3.plugins.registry import PluginInfo

@pytest.fixture(autouse=True)
def force_mock_no_gl(monkeypatch):
    monkeypatch.setattr('vjlive3.plugins.depth_mesh.HAS_GL', False)
...
def test_depth_mesh_manifest():
    manifest = PluginInfo(**METADATA)
    assert manifest.name == "Depth Mesh"
    assert "meshWireframe" in [p["name"] for p in manifest.parameters]
    assert "u_mix" in [p["name"] for p in manifest.parameters]
    assert "video_in" in manifest.inputs
    assert "depth_in" in manifest.inputs
    assert "video_out" in manifest.outputs

def test_depth_mesh_bypassed():
    plugin = DepthMeshPlugin()
    ...
    res = plugin.process_frame(100, {"meshWireframe": 1.0}, ctx)
    assert ctx.outputs["video_out"] == 100

def test_depth_mesh_missing_video():
    plugin = DepthMeshPlugin()
    ctx = MockContext()
...
def setup_mock_gl(monkeypatch):
    ...
    monkeypatch.setattr(dm, 'gl', mock_gl, raising=False)
    monkeypatch.setattr(dm, 'HAS_GL', True)
    return mock_gl

def test_depth_mesh_fbo_cleanup(monkeypatch):
...
def test_depth_mesh_gl_exception_handling(monkeypatch):
    mock_gl = setup_mock_gl(monkeypatch)
...
def test_depth_mesh_render_frame(monkeypatch):
    mock_gl = setup_mock_gl(monkeypatch)
    plugin = DepthMeshPlugin()
    plugin._mock_mode = False
    ...
    # Test wireframe active
    params["meshWireframe"] = 10.0
    res5 = plugin.process_frame(100, params, ctx)
    assert res5 == 1

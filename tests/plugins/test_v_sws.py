...
from vjlive3.plugins.v_sws import VSwsPlugin, METADATA
@pytest.fixture
def plugin():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='V Sws'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.v_sws.gl', _gl)
...
@patch('vjlive3.plugins.v_sws.gl', _gl)
@patch('vjlive3.plugins.v_sws.hasattr')

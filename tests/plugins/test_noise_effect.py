...
from vjlive3.plugins.noise_effect import NoisePlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Noise Effect'
    assert len(m["parameters"]) == 11

@patch('vjlive3.plugins.noise_effect.gl', _mock_gl)
...
@patch('vjlive3.plugins.noise_effect.gl', _mock_gl)
@patch('vjlive3.plugins.noise_effect.hasattr')

"""Tests for P7-VE58: ResizePlugin."""
import pytest
...
from vjlive3.plugins.resize_effect import ResizePlugin, METADATA
@pytest.fixture
def plugin():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Resize Effect'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.resize_effect.gl', _gl)
...
@patch('vjlive3.plugins.resize_effect.gl', _gl)
@patch('vjlive3.plugins.resize_effect.hasattr')

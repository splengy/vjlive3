"""Tests for P5-DM21: FaceMeltDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.face_melt_datamosh import FaceMeltDatamoshPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Face Melt Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.face_melt_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.face_melt_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.face_melt_datamosh.hasattr')

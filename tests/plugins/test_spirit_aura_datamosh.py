"""Tests for P5-DM30: SpiritAuraDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.spirit_aura_datamosh import SpiritAuraDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return SpiritAuraDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Spirit Aura Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.spirit_aura_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.spirit_aura_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.spirit_aura_datamosh.hasattr')

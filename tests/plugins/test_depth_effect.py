...
def force_mock_no_gl(monkeypatch):
    monkeypatch.setattr('vjlive3.plugins.depth_effect.HAS_GL', False)
...
def test_depth_effect_manifest():
    manifest = PluginInfo(**METADATA)
    assert manifest.name == "Depth Effects"
    assert "minDepth" in [p["name"] for p in manifest.parameters]
...
def test_depth_effect_bypassed():
    ...
    res = plugin.process_frame(100, {"minDepth": 1.0}, ctx)
...
def test_depth_effect_gl_exception_handling(monkeypatch):
    mock_gl = setup_mock_gl(monkeypatch)

"""Tests for P5-DM18: ShatterDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.shatter_datamosh import ShatterDatamoshPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Shatter Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.shatter_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.shatter_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.shatter_datamosh.hasattr')

"""Tests for P5-DM09: CupcakeCascadeDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.cupcake_cascade_datamosh import CupcakeCascadeDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return CupcakeCascadeDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Cupcake Cascade Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.cupcake_cascade_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.cupcake_cascade_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.cupcake_cascade_datamosh.hasattr')

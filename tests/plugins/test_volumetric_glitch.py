"""Tests for P5-DM36: VolumetricGlitchPlugin."""
import pytest
...
from vjlive3.plugins.volumetric_glitch import VolumetricGlitchPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Volumetric Glitch'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.volumetric_glitch.gl', _mock_gl)
...
@patch('vjlive3.plugins.volumetric_glitch.gl', _mock_gl)
@patch('vjlive3.plugins.volumetric_glitch.hasattr')

"""Tests for P5-DM29: SacredGeometryDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.sacred_geometry_datamosh import SacredGeometryDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return SacredGeometryDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Sacred Geometry Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.sacred_geometry_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.sacred_geometry_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.sacred_geometry_datamosh.hasattr')

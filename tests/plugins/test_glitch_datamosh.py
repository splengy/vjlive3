"""Tests for P5-DM05: GlitchDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.glitch_datamosh import GlitchDatamoshPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Glitch Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 10

@patch('vjlive3.plugins.glitch_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.glitch_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.glitch_datamosh.hasattr')

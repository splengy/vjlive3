"""Tests for P6-QC08: MLBaseAsyncPlugin."""
import pytest
...
from vjlive3.plugins.ml_base_async import MLBaseAsyncPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Ml Base Async'
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.ml_base_async.gl', _mock_gl)
...
@patch('vjlive3.plugins.ml_base_async.gl', _mock_gl)
@patch('vjlive3.plugins.ml_base_async.hasattr')

"""Tests for P5-DM17: LayerSeparationPlugin."""
import pytest
...
from vjlive3.plugins.layer_separation_datamosh import LayerSeparationPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Layer Separation Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.layer_separation_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.layer_separation_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.layer_separation_datamosh.hasattr')

...
from vjlive3.plugins.osc_effect import OscPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Osc Effect'
    assert len(m["parameters"]) == 14

@patch('vjlive3.plugins.osc_effect.gl', _mock_gl)
...
@patch('vjlive3.plugins.osc_effect.gl', _mock_gl)
@patch('vjlive3.plugins.osc_effect.hasattr')

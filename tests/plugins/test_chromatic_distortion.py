"""Tests for P7-VE32: ChromaticDistortionPlugin."""
import pytest
...
from vjlive3.plugins.chromatic_distortion import ChromaticDistortionPlugin, METADATA
...
def plugin():
    ...
    _gl.glCreateProgram.return_value=99; _gl.glGenVertexArrays.return_value=44
    _gl.glGenTextures.return_value=55; _gl.glGenFramebuffers.return_value=51
    return ChromaticDistortionPlugin()
@pytest.fixture
def context():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Chromatic Distortion'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.chromatic_distortion.gl', _gl)
...
@patch('vjlive3.plugins.chromatic_distortion.gl', _gl)
@patch('vjlive3.plugins.chromatic_distortion.hasattr')

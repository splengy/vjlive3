"""Tests for P5-DM02: BadTripDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.bad_trip_datamosh import BadTripDatamoshPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Bad Trip Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.bad_trip_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.bad_trip_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.bad_trip_datamosh.hasattr')

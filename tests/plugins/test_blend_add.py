"""Tests for P7-VE09: BlendAddPlugin."""
import pytest
...
from vjlive3.plugins.blend_add import BlendAddPlugin, METADATA
@pytest.fixture
def plugin():
    _gl.reset_mock(); _gl.glGetShaderiv.return_value=1; _gl.glGetProgramiv.return_value=1
    _gl.glCreateProgram.return_value=99; _gl.glGenVertexArrays.return_value=44
    _gl.glGenTextures.return_value=55; _gl.glGenFramebuffers.return_value=51
    return BlendAddPlugin()
@pytest.fixture
def context():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Blend Add'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.blend_add.gl', _gl)
...
@patch('vjlive3.plugins.blend_add.gl', _gl)
@patch('vjlive3.plugins.blend_add.hasattr')

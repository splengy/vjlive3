...
def test_plugin_metadata(plugin):
    ...
    assert meta["name"] == "DepthContour"
    assert meta["plugin_type"] == "depth_effect"
    assert "contourIntervals" in [p["name"] for p in meta["parameters"]]

@patch('vjlive3.plugins.depth_contour.gl', mock_gl)
...
@patch('vjlive3.plugins.depth_contour.gl', mock_gl)
@patch('vjlive3.plugins.depth_contour.hasattr')
...
def test_process_frame_standard_execution(plugin, context):
    ...
    # Interval parameter 0.5 * 50 = 25.0
    mock_gl.glUniform1f.assert_any_call(mock_gl.glGetUniformLocation(99, "intervals"), 25.0)
    
    # Thick parameter 0.2 * 10 = 2.0
    mock_gl.glUniform1f.assert_any_call(mock_gl.glGetUniformLocation(99, "thickness"), 2.0)
    
    # Assert Draw Elements is triggered
    mock_gl.glDrawElements.assert_any_call(mock_gl.GL_TRIANGLES, plugin.num_indices, mock_gl.GL_UNSIGNED_INT, None)
    
    # Test alternative color branches
    params["color_mode"] = "velocity"

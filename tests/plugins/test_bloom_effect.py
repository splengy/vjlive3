...
from vjlive3.plugins.bloom_effect import BloomPlugin, METADATA
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Bloom Effect'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.bloom_effect.gl', _gl)
...
@patch('vjlive3.plugins.bloom_effect.gl', _gl)
@patch('vjlive3.plugins.bloom_effect.hasattr')

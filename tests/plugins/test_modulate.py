"""Tests for P5-MO03: Modulate Effect."""
import pytest
...
from vjlive3.plugins.modulate import ModulatePlugin, METADATA, _map
...
def test_metadata(plugin):
    ...
    assert m["name"] == "Modulate"
    pnames = [p["name"] for p in m["parameters"]]
    assert "amount" in pnames; assert "direction" in pnames
    assert "iterations" in pnames; assert len(pnames) == 8

def test_map_limits(plugin):
    assert _map(0.0, 0.0, 1.0) == pytest.approx(0.0); assert _map(10.0, 0.0, 1.0) == pytest.approx(1.0)

@patch('vjlive3.plugins.modulate.gl', _mock_gl)
...
@patch('vjlive3.plugins.modulate.gl', _mock_gl)
@patch('vjlive3.plugins.modulate.hasattr')
...
@patch('vjlive3.plugins.modulate.gl', _mock_gl)
def test_process_all_directions(plugin, context):
    plugin.initialize(context)
    for dir_val in [0.0, 3.0, 5.0, 7.0, 10.0]:
        _mock_gl.reset_mock()
        _mock_gl.glGetShaderiv.return_value = 1; _mock_gl.glGetProgramiv.return_value = 1
        _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
        _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
        params = {"amount": 5.0, "direction": dir_val, "iterations": 3.0}
        assert plugin.process_frame(10, params, context) == 55
...
def test_cleanup(plugin, context):
    plugin.initialize(context); plugin.prog = 99; plugin.trail_fbo = 51; plugin.cleanup()

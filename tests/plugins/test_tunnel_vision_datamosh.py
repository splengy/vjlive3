"""Tests for P5-DM32: TunnelVisionDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.tunnel_vision_datamosh import TunnelVisionDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return TunnelVisionDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Tunnel Vision Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.tunnel_vision_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.tunnel_vision_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.tunnel_vision_datamosh.hasattr')

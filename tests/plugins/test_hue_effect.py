...
from vjlive3.plugins.hue_effect import HuePlugin, METADATA
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Hue Effect'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.hue_effect.gl', _gl)
...
@patch('vjlive3.plugins.hue_effect.gl', _gl)
@patch('vjlive3.plugins.hue_effect.hasattr')

"""Tests for P5-DM13: MeltDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.melt_datamosh import MeltDatamoshPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Melt Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.melt_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.melt_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.melt_datamosh.hasattr')

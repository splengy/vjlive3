"""Tests for P7-VE19: ChromaKeyPlugin."""
import pytest
...
from vjlive3.plugins.chroma_key import ChromaKeyPlugin, METADATA
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Chroma Key'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.chroma_key.gl', _gl)
...
@patch('vjlive3.plugins.chroma_key.gl', _gl)
@patch('vjlive3.plugins.chroma_key.hasattr')

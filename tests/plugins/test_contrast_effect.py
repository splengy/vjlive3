"""Tests for P7-VE21: ContrastPlugin."""
import pytest
...
from vjlive3.plugins.contrast_effect import ContrastPlugin, METADATA
@pytest.fixture
def plugin():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Contrast Effect'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.contrast_effect.gl', _gl)
...
@patch('vjlive3.plugins.contrast_effect.gl', _gl)
@patch('vjlive3.plugins.contrast_effect.hasattr')

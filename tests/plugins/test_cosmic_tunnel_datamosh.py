"""Tests for P4-AU06: Cosmic Tunnel Datamosh."""
import pytest
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return CosmicTunnelDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == "Cosmic Tunnel Datamosh"
    pnames = [p["name"] for p in m["parameters"]]
    assert "tunnel_speed" in pnames; assert "recursion" in pnames; assert len(pnames) == 11

def test_map_limits(plugin):
    assert _map(0.0, 0.0, 1.0) == pytest.approx(0.0); assert _map(10.0, 0.0, 1.0) == pytest.approx(1.0)

@patch('vjlive3.plugins.cosmic_tunnel_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.cosmic_tunnel_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.cosmic_tunnel_datamosh.hasattr')
...
def test_process_renders(plugin, context):
    ...
    context.inputs["audio_data"] = {"bass": 0.7, "volume": 0.8}
    params = {"tunnel_speed": 6.0, "recursion": 7.0, "grid_lines": 3.0}
    res = plugin.process_frame(10, params, context)
    assert res == 55; _mock_gl.glDrawArrays.assert_called_once()

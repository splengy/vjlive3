"""Tests for P5-DM15: FrameHoldDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.frame_hold_datamosh import FrameHoldDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return FrameHoldDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Frame Hold Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.frame_hold_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.frame_hold_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.frame_hold_datamosh.hasattr')

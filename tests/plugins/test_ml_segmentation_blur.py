"""Tests for P6-QC10: MLSegmentationBlurPlugin."""
import pytest
...
from vjlive3.plugins.ml_segmentation_blur import MLSegmentationBlurPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return MLSegmentationBlurPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Ml Segmentation Blur'
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.ml_segmentation_blur.gl', _mock_gl)
...
@patch('vjlive3.plugins.ml_segmentation_blur.gl', _mock_gl)
@patch('vjlive3.plugins.ml_segmentation_blur.hasattr')

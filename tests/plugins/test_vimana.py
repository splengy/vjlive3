"""Tests for P7-VE77: VimanaPlugin."""
import pytest
...
from vjlive3.plugins.vimana import VimanaPlugin, METADATA
@pytest.fixture
def plugin():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Vimana'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.vimana.gl', _gl)
...
@patch('vjlive3.plugins.vimana.gl', _gl)
@patch('vjlive3.plugins.vimana.hasattr')

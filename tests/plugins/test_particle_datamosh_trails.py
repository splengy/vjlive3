"""Tests for P5-DM26: ParticleDatamoshTrailsPlugin."""
import pytest
...
from vjlive3.plugins.particle_datamosh_trails import ParticleDatamoshTrailsPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return ParticleDatamoshTrailsPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Particle Datamosh Trails'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.particle_datamosh_trails.gl', _mock_gl)
...
@patch('vjlive3.plugins.particle_datamosh_trails.gl', _mock_gl)
@patch('vjlive3.plugins.particle_datamosh_trails.hasattr')

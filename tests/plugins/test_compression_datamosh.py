"""Tests for P5-DM10: CompressionDatamoshPlugin."""
import pytest
...
from vjlive3.plugins.compression_datamosh import CompressionDatamoshPlugin, METADATA
...
def plugin():
    ...
    _mock_gl.glCreateProgram.return_value = 99; _mock_gl.glGenVertexArrays.return_value = 44
    _mock_gl.glGenTextures.return_value = 55; _mock_gl.glGenFramebuffers.return_value = 51
    return CompressionDatamoshPlugin()
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Compression Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 12

@patch('vjlive3.plugins.compression_datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.compression_datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.compression_datamosh.hasattr')

"""Tests for P7-VE78: VimanaHyperionUltimatePlugin."""
import pytest
...
from vjlive3.plugins.vimana_hyperion_ultimate import VimanaHyperionUltimatePlugin, METADATA
...
def plugin():
    ...
    _gl.glCreateProgram.return_value=99; _gl.glGenVertexArrays.return_value=44
    _gl.glGenTextures.return_value=55; _gl.glGenFramebuffers.return_value=51
    return VimanaHyperionUltimatePlugin()
@pytest.fixture
def context():
...
def test_metadata(plugin):
    m=plugin.get_metadata(); assert m["name"]=='Vimana Hyperion Ultimate'; assert len(m["parameters"])==6
@patch('vjlive3.plugins.vimana_hyperion_ultimate.gl', _gl)
...
@patch('vjlive3.plugins.vimana_hyperion_ultimate.gl', _gl)
@patch('vjlive3.plugins.vimana_hyperion_ultimate.hasattr')

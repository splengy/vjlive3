"""Tests for P5-DM11: DatamoshPlugin."""
import pytest
...
from vjlive3.plugins.datamosh import DatamoshPlugin, METADATA
...
def test_metadata(plugin):
    ...
    assert m["name"] == 'Datamosh'
    assert "video_in" in m["inputs"]
    assert len(m["parameters"]) == 8

@patch('vjlive3.plugins.datamosh.gl', _mock_gl)
...
@patch('vjlive3.plugins.datamosh.gl', _mock_gl)
@patch('vjlive3.plugins.datamosh.hasattr')

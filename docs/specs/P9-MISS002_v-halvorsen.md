...
## Task: P9-MISS002 — V-Halvorsen (VHalvorsenPlugin)
...
VHalvorsenPlugin is a chaotic attractor module that generates continuously evolving X/Y/Z/T outputs using the Halvorsen differential equations. It produces intricate spiral patterns with parameter `a` that create non-repeating, organic trajectories. The attractor's mathematical properties yield smooth, flowing motion ideal for modulating visual parameters with a more circular, spiral-like quality compared to the Lorenz butterfly pattern.

## What It Does NOT Do

- Does not generate visual output directly (uses dummy shader)
- Does not provide real-time parameter editing during playback
- Does not support multiple attractor types (only Halvorsen)
...
class VHalvorsenPlugin(Effect):
    def __init__(self) -> None:
        """
        Initialize the Halvorsen attractor plugin with default parameters.
        
        Creates a HalvorsenAttractor instance and sets up default
    ...
    def get_state(self) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - 'a': Halvorsen parameter 'a' (1.43 default)
        """
        pass
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - 'a': Halvorsen parameter 'a'
        """
        pass
    
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> 'VHalvorsenPlugin':
        """
...
- **Parameter out of range**: Parameters are clamped to valid ranges (0.0-10.0 for speed/amplitude, 'a' parameter has reasonable range)
...
- **Internal modules this depends on**:
  ...
  - `core.modulation_attractors.HalvorsenAttractor` — core attractor engine
...
| `test_serialization` | to_dict and from_dict preserve all parameters including 'a' |
...
- [ ] Git commit with `[Phase-9] P9-MISS002: V-Halvorsen plugin implementation` message
- [ ] BOARD.md updated
- [ ] Lock released
...
- The Halvorsen attractor uses the equations: dx=(-a*x-4*y-4*z-y²), dy=(-a*y-4*z-4*x-z²), dz=(-a*z-4*x-4*y-x²)
- Default parameter: a=1.43 (classic Halvorsen spiral chaos)

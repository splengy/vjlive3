# P7-VE79: Vimana Synth Effect

> **Task ID:** `P7-VE79`
> **Priority:** P0
> **Source:** VJlive-2 (`plugins/vvimana/vimana_synth.py`)
> **Class:** `VimanaEffect`
...
Base synthesizer effect: audio-reactive visual synthesis with configurable oscillators and filters. Creates evolving textures and patterns. Essential for audio-reactive visuals.
...
```python
def remap_params(oscillator_count, frequency_base, filter_cutoff, modulation_depth, opacity):
...
def vimana_cpu(frame, oscillator_count, frequency_base, filter_cutoff, modulation_depth, opacity):
    ...
    try:
        ...
        print(f"vimana_cpu error: {e}")
        return frame
```

## Presets

| Name | oscillator_count | frequency_base | filter_cutoff | modulation_depth | opacity |
|------|-----------------|----------------|---------------|-------------------|---------|
| Rich Texture | 5 | 5 | 5 | 5 | 10 |
| Deep Bass | 3 | 2 | 3 | 3 | 9 |
| High Pitched | 7 | 8 | 7 | 4 | 8 |
| Evolving | 6 | 6 | 6 | 6 | 10 |
...
class VimanaEffect(Effect):
    """Base audio-reactive synthesizer with configurable oscillators and filters.
    
    Attributes:
        effect_category: "synthesizer"
        parameters: {oscillator_count, frequency_base, filter_cutoff, modulation_depth, opacity}
    ...
    def __init__(self, width=1920, height=1080, use_gpu=True):
        super().__init__("Vimana", vert_shader, frag_shader)
...
low_count = vimana_cpu(test_image, 2, 5, 5, 5, 1.0)
high_count = vimana_cpu(test_image, 6, 5, 5, 5, 1.0)
...
low_freq = vimana_cpu(test_image, 3, 2, 5, 5, 1.0)    # 40Hz
high_freq = vimana_cpu(test_image, 3, 8, 5, 5, 1.0)   # 1600Hz
...
low_mod = vimana_cpu(test_image, 3, 5, 5, 2, 1.0)    # depth=0.2
high_mod = vimana_cpu(test_image, 3, 5, 5, 8, 1.0)   # depth=0.8
...
- [x] Tests (oscillators, frequency, modulation, opacity)

**Estimated Coverage:** 82% (synthesis math, parameter validation, presets, edge cases)
...
**Artisanal Note:** Base synthesizer creates audio-reactive patterns through multiple oscillators, filters, and modulation. Frequency base controls pitch, filter cutoff shapes timbre, modulation adds variation. CPU fallback uses simplified synthesis. Presets calibrated for rich textures, deep bass, high-pitched patterns, and evolving sequences.
- No silent failures, proper error handling (Safety Rail 7)

## Implementation Notes

**Original Location:** `VJlive-2/plugins/vvimana/vimana_synth.py`
**Description:** No description available

### Porting Strategy

1. Study the original implementation in `VJlive-2/plugins/vvimana/vimana_synth.py`
...
- Original source: `VJlive-2/plugins/vvimana/vimana_synth.py`

...
### [NEEDS RESEARCH]: Fallback implementation details when numpy is missing

**Finding**: The spec states `numpy` is required and fallback is [NEEDS RESEARCH]. However, background subtraction without numpy is infeasible in pure Python for real-time video—it would be orders of magnitude too slow. The fallback should assume numpy is present. If numpy is missing, the effect should raise an ImportError at module import time, not attempt a pure-Python implementation. The `test_init_no_hardware` test likely refers to OpenCV absence, not numpy absence. Update the spec: "If OpenCV is missing, fall back to CPU-based numpy processing; if numpy is missing, the effect cannot function and should raise ImportError."

**Resolution**: The Dependencies section should be updated to reflect that `numpy` is a hard requirement with no fallback. The fallback only applies to OpenCV.

### [NEEDS RESEARCH]: How the effect integrates with VJLive3's effects system (registry, parameter handling)

**Finding**: The legacy code imports from `plugins.vcore.background_subtraction` and registers in `core.effects.ALL_EFFECTS`. The VJLive3 effect system likely expects effects to inherit from a base `Effect` class that provides `set_parameter`, `get_parameter`, and `process_frame` (or `apply`). The skeleton spec's `__init__(param: dict)` is atypical—most effects use `__init__()` with optional keyword args. The implementation should adapt: accept either a dict or explicit parameters. The base class may handle parameter storage and validation. The effect should also define a `METADATA` manifest for automatic UI generation, similar to the Vimana example.

**Resolution**: The spec should define a standard `Effect` base class interface. The `BackgroundSubtractionEffect` should inherit from it and use `METADATA` for parameter definitions. The `__init__` can accept `**kwargs` or a config dict; both are acceptable.

### [NEEDS RESEARCH]: Output format of process_frame: single-channel mask vs RGB-composited image

**Finding**: The legacy test suggests the effect has `silhouetteColor` and `opacity` parameters, implying it can produce an RGB overlay. However, the skeleton spec says "motion-difference image" with same shape as input. This is ambiguous. The most flexible design is to output a single-channel mask (uint8, 0-255) and let a separate effect handle colorization and blending. This follows the single-responsibility principle. The `silhouetteColor` and `opacity` parameters can be omitted from the VJLive3 version, or they can be used by a downstream `MaskCompositorEffect`. Recommendation: Output single-channel mask; document that colorization is out of scope.

**Resolution**: The spec should explicitly state that `process_frame` returns a single-channel (H, W) uint8 mask. The `silhouetteColor` and `opacity` parameters are optional and may be used by downstream effects; they are not used by this effect directly.

### [NEEDS RESEARCH]: Exact mapping of `threshold` parameter: is it 0-1 normalized or 0-255?

**Finding**: The legacy test sets `threshold` to 0.7 and 2.0, and checks clamping to 1.0. This suggests a normalized 0-1 range. However, OpenCV's `varThreshold` is typically in the range 0-100 (or higher). The mapping should be: `varThreshold = threshold * 100` if threshold is normalized. For fallback mode, `pixel_threshold = threshold * 255`. The spec should clarify that `threshold` is a normalized float in [0.0, 1.0] and internally mapped appropriately.

**Resolution**: The spec should define `threshold` as a normalized float in [0.0, 1.0] with default 0.25. Internally, map to OpenCV's varThreshold as `threshold * 100` and to fallback pixel threshold as `threshold * 255`.

---

## Configuration Schema

The effect should define a `METADATA` manifest:

```python
METADATA = {
  "params": [
    {"id": "use_opencv", "name": "Use OpenCV", "default": True, "type": "bool", "description": "Use OpenCV's BackgroundSubtractor if available (read-only after init)"},
    {"id": "history_frames", "name": "History Length", "default": 50, "min": 1, "max": 500, "type": "int", "description": "Number of frames to average for background model"},
    {"id": "threshold", "name": "Detection Threshold", "default": 0.25, "min": 0.0, "max": 1.0, "type": "float", "description": "Sensitivity of motion detection (0=all motion, 1=only large motion). Normalized; internally mapped to 0-255 or OpenCV varThreshold."},
    {"id": "blur", "name": "Blur Radius", "default": 2.0, "min": 0.0, "max": 10.0, "type": "float", "description": "Gaussian blur radius for mask smoothing (0=no blur). Converted to odd integer kernel size: 2*int(blur)+1"},
    {"id": "opacity", "name": "Overlay Opacity", "default": 1.0, "min": 0.0, "max": 1.0, "type": "float", "description": "Opacity of colored silhouette when compositing (used by downstream effects)"},
    {"id": "silhouette_color", "name": "Silhouette Color", "default": [1.0, 0.0, 0.0], "min": 0.0, "max": 1.0, "type": "list[float]", "description": "RGB color for motion silhouette (used by downstream effects)"},
  ]
}
```

Note: The `use_opencv` parameter is read-only after initialization; changing it requires a reset. The `blur` parameter should be converted to an odd integer kernel size: `kernel_size = 2 * int(blur) + 1`.

---

## State Management

- **Per-frame state**: The current frame being processed, the computed mask, and temporary buffers. These are transient and discarded after `process_frame` returns.
- **Persistent state**: The background model (`self.subtractor` or `self.background`), learning rate, frame count, and configuration parameters. These persist across frames and must be serializable if preset saving is required.
- **Init-once state**: OpenCV subtractor instance (expensive to create), numpy arrays for background buffer. Initialized in `__init__` or first call to `process_frame`.
- **Thread safety**: The effect is not thread-safe by default. If the pipeline calls `process_frame` from multiple threads, external synchronization is required. Alternatively, the effect could use a lock around its state updates.

---

## GPU Resources

This effect is CPU-bound and does not use GPU resources directly. It operates on numpy arrays in system memory. If the pipeline uses GPU textures, the effect will need to handle texture readback (glReadPixels) and upload (glTexImage2D), but that is the pipeline's responsibility, not the effect's. The effect's contract is: input numpy array → output numpy array.
...
class BackgroundSubtractionEffect:
    def __init__(self, param: dict) -> None:
        """
        Initialize the background subtraction effect.
        
        Args:
            param: Configuration dictionary containing parameters like:
                - 'use_opencv': bool, whether to use OpenCV's BGSAF method (default: True)
                - 'history_frames': int, number of frames for background history (default: 50)
                - 'threshold': float, threshold value for motion detection (default: 30.0)
        """
    
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        """
        Process a single video frame and return the difference image.
        
        Args:
            frame: Input RGB or grayscale numpy array of shape (H, W, C)
            
        Returns:
            np.ndarray: Motion-difference image (same shape as input), with pixel values indicating motion intensity
        """
    
    def reset(self) -> None:
        """Reset internal state to reinitialize background model."""
```
...
|------|------|-------------|-------------|
| `param` | `dict` | Configuration parameters for the effect | Must contain at least 'history_frames' (int ≥ 1), 'threshold' (float ≥ 0). Default values: history_frames=50, threshold=30.0. 'use_opencv' defaults to True |
| `frame` | `np.ndarray` | Input video frame in RGB or grayscale format | Shape must be (H, W, C) where H≥1, W≥1, C=3 or 1. Values in [0, 255] for uint8. Invalid frames raise ValueError |
| `output_frame` | `np.ndarray` | Output motion-difference image | Same shape as input frame; pixel values represent motion intensity (0 = no motion, higher = more motion) |
...
- External libraries needed (and what happens if they are missing):
  - `numpy` — used for array operations in frame processing — **hard requirement**; if missing, raises ImportError at module import
  - `cv2` — used for background subtraction via OpenCV's BGSubtractor methods — fallback: fall back to basic pixel-wise difference with thresholding
- Internal modules this depends on:
  - `core.effects.ALL_EFFECTS` — registers the effect in the global effects registry; must be present and contain 'background_subtraction' key
  - `plugins.vcore.background_subtraction` — legacy module used for import path validation
...
| Test Name | What It Verifies |
|-----------|-----------------|
| `test_init_no_hardware` | Module starts without crashing if OpenCV or NumPy is missing; falls back to basic logic |
| `test_basic_operation` | Process frame returns valid output with motion differences when input is valid and parameters are default |
| `test_error_handling` | Invalid frame shape (e.g., 0x0) raises ValueError; invalid parameter values (e.g., negative history_frames) raise TypeError |

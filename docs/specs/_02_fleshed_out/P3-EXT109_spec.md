...
### [NEEDS RESEARCH]: What exactly does `get_tempos()` return? Beat positions (0-1) or BPM values?

**Finding**: The legacy manifest shows parameters but not the method signature. The skeleton spec says "Return current tempo values for each channel (1–6)" and returns `dict[int, float]`. In modular synths, a "tempo" could mean BPM or phase. Given that the module is called "tempo" and has a `get_tempos()` method, it likely returns the actual BPM for each channel (the derived tempo). However, beat position is also useful for triggering.

**Resolution**: Define `get_tempos()` to return a dict mapping channel number to its **current BPM** (float). For beat phase, provide a separate method `get_beat_positions()` or `get_phases()`. Alternatively, return both in a tuple. To keep it simple, `get_tempos()` returns BPMs; if beat phase is needed, call `get_beat_phase(channel)`.
...
- **Thread safety**: The module is **not thread-safe** by default. If `update()` and `get_tempos()` are called from different threads, external synchronization (e.g., a lock) is required. For typical VJLive3 usage, all calls happen on the main thread, so this is not an issue.
...
class VTempiPlugin:
    def __init__(self, config: dict = None) -> None:
        """
        Initialize the V-Tempi plugin with optional configuration.
        
        Args:
            config: Dictionary containing configuration parameters like 'leading_tempo' and 'video_fps'
        """
    
    def update(self, delta_time: float) -> None:
        """
        Update the internal state of the tempo engine based on time delta.
        
        Args:
            delta_time: Time elapsed since last update in seconds
        """
    
    def get_tempos(self) -> dict:
        """
        Return current tempo values for each channel (1–6).
        
        Returns:
            Dictionary mapping channel number to tempo value (BPM)
        """
    
    def set_tempo(self, channel: int, bpm: float) -> None:
        """
        Set the BPM for a specific channel.
        
        Args:
            channel: Channel index (1-6)
            bpm: Tempo in beats per minute
        """
    
    def get_video_fps(self) -> float:
        """
        Return current video frame rate setting.
    ...
    def set_video_fps(self, fps: float) -> None:
        """
        Update the video frame rate for alignment calculations.
        
        Args:
            fps: Frames per second (1.0 to 120.0)
        """
    
    def get_beat_phase(self, channel: int) -> float:
        """
        Get the current beat phase (0.0-1.0) for a channel.
        
        Args:
            channel: Channel index (1-6)
            
        Returns:
            Beat phase as a float in [0.0, 1.0)
        """
    
    def tap(self, channel: int = 1) -> None:
        """
        Register a tap event for tap tempo (typically bound to a button).
...
|------|------|-------------|-------------|
| `config` | `dict` | Configuration dictionary passed at initialization | Must contain 'leading_tempo' (float, ≥0) and optional 'video_fps' (float, ≥1.0) |
| `delta_time` | `float` | Time delta in seconds between updates | Must be ≥ 0; typically from VJLive3's time loop |
| `channel` | `int` | Channel index to modify (1–6) | Range: 1 to 6 inclusive |
| `bpm` | `float` | Tempo value in beats per minute | Range: 0.0 to 500.0; invalid values raise ValueError |
| `fps` | `float` | Video frame rate | Range: 1.0 to 120.0; invalid values raise ValueError |
| `return_value` (from get_tempos) | `dict[int, float]` | Current tempo per channel | Keys: 1–6; values: ≥0.0, derived from leading_tempo and channel-specific modulation |
| `return_value` (from get_beat_phase) | `float` | Current beat phase | In [0.0, 1.0) |
...
- Internal modules this depends on:
  ...
  - `vjlive3.utils.config_parser.ConfigParser` — to validate input config against schema
...
| `test_mute_channel` | Muted channel returns beat position 0 or None (specify) |
| `test_tap_tempo_ch1` | Tap tempo updates leading_tempo based on tap intervals |
| `test_tap_tempo_smoothing` | Tap tempo applies smoothing to avoid jitter |
...
- [ ] Git commit with `[Phase-X] P3-EXT109: vtempi (ModBehavior) - Port from VJLive legacy to VJLive3` message
- [ ] BOARD.md updated
- [ ] Lock released

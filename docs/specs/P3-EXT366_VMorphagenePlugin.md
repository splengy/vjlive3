...
A direct software emulation of the hardware Make Noise Morphagene Eurorack synthesizer module. It translates physical CV (Control Voltage) and audio-rate behavior into the VJLive3 virtual graph, maintaining strict adherence to the Make Noise philosophy.

## What This Module Does

This specification defines the VJLive3 port for the legacy `vmake_noise` ecosystem plugin: `VMorphagenePlugin`. It behaves as a NodeGraph component handling input/output audio or modulation streams identically to its real-world counterpart.
...
*   **Logic (Pytest)**: Send zeroed CV signals and verify the internal physics match the resting state of the Morphagene hardware.

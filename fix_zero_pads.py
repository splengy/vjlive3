import os
import glob
import re

board_path = "BOARD.md"
fleshed_out_dir = "docs/specs/_02_fleshed_out"

fleshed_files = [os.path.basename(f) for f in glob.glob(f"{fleshed_out_dir}/*.md")]

file_core_ids = set()
for mf in fleshed_files:
    if mf.startswith("_"): continue
    
    parts = mf.split("_")[0].split("-")
    if len(parts) > 1:
        core = parts[-1]
        if ".md" in core:
            core = core.replace(".md", "")
        
        # Strip leading zeros for matching (VD01 -> VD1)
        # We need to separate the letters from the numbers first
        match = re.match(r'([A-Za-z]+)(0+)([1-9][0-9]*)', core)
        if match:
            letters = match.group(1)
            numbers = match.group(3)
            stripped_core = f"{letters}{numbers}"
            file_core_ids.add(stripped_core)
        else:
            file_core_ids.add(core)

with open(board_path, "r") as f:
    board_lines = f.readlines()

new_lines = []
modified = 0

for line in board_lines:
    if line.strip().startswith("|") and "⬜ PENDING SKELETON" in line:
        parts = line.split("|")
        if len(parts) > 2:
            spec_id = parts[1].strip()
            board_core = spec_id.split("-")[-1]
            
            # Check if this board core exists in our stripped list
            if board_core in file_core_ids:
                print(f"Zero-pad Board update: Upgrading {spec_id} (Core: {board_core})")
                line = line.replace("⬜ PENDING SKELETON (Pass 1)", "🟩 COMPLETING PASS 2 | ✅ Spec exists in _02_fleshed_out")
                line = re.sub(r'\| \*(.*)\* \|', '| \\1 |', line)
                modified += 1
                
    new_lines.append(line)

with open(board_path, "w") as f:
    f.writelines(new_lines)

print(f"Modified {modified} lines in BOARD.md via zero-pad matching.")

"""
P7-VE54: Warhol quad — 4-panel Warhol-style color repeat.
...
METADATA = {
    "name": "Warhol Quad",
    "description": "Warhol quad — 4-panel Warhol-style color repeat.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['warhol', 'pop-art', 'quad', 'color', 'silkscreen'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'hue_a', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_b', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_c', 'type': 'float', 'default': 6.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_d', 'type': 'float', 'default': 9.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float hue_a;  // Panel A hue
uniform float hue_b;  // Panel B hue
uniform float hue_c;  // Panel C hue
uniform float hue_d;  // Panel D hue
...
void main() {
    ...
    c.r *= 1.+hue_a/10.0*0.2; c.g *= 1.+saturation/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['hue_a', 'hue_b', 'hue_c', 'hue_d', 'saturation', 'mix']
_PARAM_DEFAULTS = {'hue_a': 0.0, 'hue_b': 3.0, 'hue_c': 6.0, 'hue_d': 9.0, 'saturation': 8.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class WarholQuadPlugin(EffectPlugin):
    """Warhol quad — 4-panel Warhol-style color repeat."""
    def __init__(self):
        super().__init__()

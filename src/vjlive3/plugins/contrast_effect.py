"""
P7-VE21: Contrast — levels-style contrast control.
...
METADATA = {
    ...
    "description": "Contrast — levels-style contrast control.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['contrast', 'levels', 'color', 'exposure', 'grade'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'contrast', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'brightness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'black', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'white', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float black;  // Black level
uniform float white;  // White level
...
_PARAM_NAMES = ['contrast', 'brightness', 'gamma', 'black', 'white', 'mix']
_PARAM_DEFAULTS = {'contrast': 5.0, 'brightness': 5.0, 'gamma': 5.0, 'black': 0.0, 'white': 10.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ContrastPlugin(EffectPlugin):
    """Contrast — levels-style contrast control."""
    def __init__(self):
        super().__init__()

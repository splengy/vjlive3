"""
P6-GE18: Granular video — scrubs video in granular synthesis style.
...
METADATA = {
    "name": "Granular Video",
    "description": "Granular video — scrubs video in granular synthesis style.",
    "version": "1.0.0",
    "plugin_type": "generator",
    "category": "generator",
    "tags": ['granular', 'video-scrub', 'temporal', 'generative', 'experimental'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'grain_size', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'grain_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'time_spread', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'position_spread', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pitch_shift', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'chaos', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'envelope', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'reverse_prob', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'freeze_point', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_scatter', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float grain_size;  // Grain duration
uniform float grain_density;  // Grain count
uniform float time_spread;  // Time scatter
uniform float position_spread;  // Position scatter
uniform float pitch_shift;  // Temporal pitch
uniform float feedback;  // Feedback
uniform float chaos;  // Grain chaos
uniform float envelope;  // Grain envelope
uniform float reverse_prob;  // Reverse chance
uniform float freeze_point;  // Freeze point
uniform float color_scatter;  // Color scatter
uniform float blend;  // Blend mode
...
void main() {
    ...
    float disp     = (grain_size / 10.0);
    float feed     = (feedback / 10.0);
    float freq_v   = 8.0 + (chaos / 10.0) * 10.0;
    float chroma   = (color_scatter / 10.0);
    float vign_v   = 0.0;
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + feedback/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['grain_size', 'grain_density', 'time_spread', 'position_spread', 'pitch_shift', 'feedback', 'chaos', 'envelope', 'reverse_prob', 'freeze_point', 'color_scatter', 'blend']
_PARAM_DEFAULTS = {'grain_size': 5.0, 'grain_density': 5.0, 'time_spread': 5.0, 'position_spread': 5.0, 'pitch_shift': 5.0, 'feedback': 3.0, 'chaos': 3.0, 'envelope': 5.0, 'reverse_prob': 0.0, 'freeze_point': 5.0, 'color_scatter': 3.0, 'blend': 5.0}
...
class GranularVideoPlugin(EffectPlugin):

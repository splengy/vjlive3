"""
P7-VE32: Chromatic distortion — per-channel UV distortion.
...
METADATA = {
    ...
    "description": "Chromatic distortion — per-channel UV distortion.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['chromatic', 'distortion', 'aberration', 'uv', 'glitch'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pattern', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_boost', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'frequency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float strength;  // Distortion strength
uniform float pattern;  // Distortion pattern
uniform float speed;  // Animation speed
uniform float edge_boost;  // Edge emphasis
uniform float frequency;  // Pattern frequency
...
void main() {
    ...
    float d = (strength/10.0); float fb = 0.0; float fr = 8.0+(frequency/10.0)*10.0;
...
_PARAM_NAMES = ['strength', 'pattern', 'speed', 'edge_boost', 'frequency', 'mix']
_PARAM_DEFAULTS = {'strength': 5.0, 'pattern': 0.0, 'speed': 3.0, 'edge_boost': 3.0, 'frequency': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ChromaticDistortionPlugin(EffectPlugin):
    """Chromatic distortion — per-channel UV distortion."""
    def __init__(self):
        super().__init__()

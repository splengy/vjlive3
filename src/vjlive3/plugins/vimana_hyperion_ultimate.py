"""
P7-VE78: Vimana Hyperion Ultimate — maximum power sacred craft mode.
...
METADATA = {
    "name": "Vimana Hyperion Ultimate",
    "description": "Vimana Hyperion Ultimate — maximum power sacred craft mode.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['vimana', 'hyperion', 'ultimate', 'sacred', 'supreme'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'power_level', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sacred_scale', 'type': 'float', 'default': 7.0, 'min': 0.0, 'max': 10.0}, {'name': 'cosmic_glow', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float power_level;  // Power level
uniform float rotation_speed;  // Spin speed
uniform float color_hue;  // Hue
uniform float sacred_scale;  // Sacred scale
uniform float cosmic_glow;  // Cosmic glow
...
void main() {
    ...
    float d = (power_level/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+power_level/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+power_level/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['power_level', 'rotation_speed', 'color_hue', 'sacred_scale', 'cosmic_glow', 'mix']
_PARAM_DEFAULTS = {'power_level': 8.0, 'rotation_speed': 5.0, 'color_hue': 5.0, 'sacred_scale': 7.0, 'cosmic_glow': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class VimanaHyperionUltimatePlugin(EffectPlugin):

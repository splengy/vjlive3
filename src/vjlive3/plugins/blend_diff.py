"""
P7-VE11: Difference blend — absolute difference of two inputs.
...
METADATA = {
    "name": "Blend Diff",
    "description": "Difference blend — absolute difference of two inputs.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['blend', 'difference', 'composite', 'subtract', 'contrast'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amount', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'boost', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'threshold', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'invert', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float amount;  // Blend amount
uniform float boost;  // Difference boost
uniform float threshold;  // Min threshold
uniform float invert;  // Invert output
uniform float color_shift;  // Hue from difference
...
void main() {
    ...
    float d = (amount/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
...
_PARAM_NAMES = ['amount', 'boost', 'threshold', 'invert', 'color_shift', 'mix']
_PARAM_DEFAULTS = {'amount': 8.0, 'boost': 5.0, 'threshold': 0.0, 'invert': 0.0, 'color_shift': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BlendDiffPlugin(EffectPlugin):
    """Difference blend — absolute difference of two inputs."""
    def __init__(self):
        super().__init__()

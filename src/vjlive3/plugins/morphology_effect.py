"""
P7-VE50: Morphology — erode/dilate/open/close operations.
...
METADATA = {
    "name": "Morphology Effect",
    "description": "Morphology — erode/dilate/open/close operations.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['morphology', 'erode', 'dilate', 'mask', 'process'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'operation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'kernel_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'iterations', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'channel', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float operation;  // 0=erode,5=dilate,8=open,10=close
uniform float kernel_size;  // Kernel size
uniform float iterations;  // Iterations
uniform float channel;  // Channel
uniform float threshold;  // Input threshold
...
_PARAM_NAMES = ['operation', 'kernel_size', 'iterations', 'channel', 'threshold', 'mix']
_PARAM_DEFAULTS = {'operation': 5.0, 'kernel_size': 3.0, 'iterations': 3.0, 'channel': 5.0, 'threshold': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class MorphologyPlugin(EffectPlugin):

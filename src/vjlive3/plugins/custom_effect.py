"""
P7-VE52: Custom effect — user scriptable plugin template.
...
METADATA = {
    "name": "Custom Effect",
    "description": "Custom effect — user scriptable plugin template.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['custom', 'template', 'user', 'scriptable', 'extend'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'param_a', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'param_b', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'param_c', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'param_d', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'param_e', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float param_a;  // Custom param A
uniform float param_b;  // Custom param B
uniform float param_c;  // Custom param C
uniform float param_d;  // Custom param D
uniform float param_e;  // Custom param E
...
_PARAM_NAMES = ['param_a', 'param_b', 'param_c', 'param_d', 'param_e', 'mix']
_PARAM_DEFAULTS = {'param_a': 5.0, 'param_b': 5.0, 'param_c': 5.0, 'param_d': 5.0, 'param_e': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class CustomEffectPlugin(EffectPlugin):
    """Custom effect — user scriptable plugin template."""
    def __init__(self):
        super().__init__()

"""
P7-VE35: Fluid simulation — GPU Navier-Stokes fluid.
...
METADATA = {
    "name": "Fluid Sim",
    "description": "Fluid simulation — GPU Navier-Stokes fluid.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['fluid', 'navier-stokes', 'simulation', 'physics', 'flow'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'viscosity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'velocity_decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'diffuse', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'force_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'curl', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float viscosity;  // Fluid viscosity
uniform float velocity_decay;  // Velocity decay
uniform float diffuse;  // Color diffusion
uniform float force_strength;  // External force
uniform float curl;  // Curl noise
...
_PARAM_NAMES = ['viscosity', 'velocity_decay', 'diffuse', 'force_strength', 'curl', 'mix']
_PARAM_DEFAULTS = {'viscosity': 5.0, 'velocity_decay': 5.0, 'diffuse': 5.0, 'force_strength': 5.0, 'curl': 3.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class FluidSimPlugin(EffectPlugin):
    """Fluid simulation — GPU Navier-Stokes fluid."""
    def __init__(self):
        super().__init__()

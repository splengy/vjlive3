"""
P6-QC13: Trails effect — frame persistence blur creating motion trails.
...
METADATA = {
    "name": "Trails Effect",
    "description": "Trails effect — frame persistence blur creating motion trails.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['trails', 'persistence', 'temporal', 'motion', 'feedback'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'trail_length', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'blur_amount', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'zoom', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_drift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'chromatic_drift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'freeze_gate', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float trail_length;  // Trail duration
uniform float decay;  // Decay rate
uniform float blur_amount;  // Blur per frame
uniform float zoom;  // Zoom into trail
uniform float rotation;  // Rotation per frame
uniform float hue_drift;  // Hue rotation
uniform float chromatic_drift;  // Chromatic drift
uniform float freeze_gate;  // Luma gate
...
void main() {
    ...
    float chroma   = (chromatic_drift / 10.0);
    float vign_v   = 0.0;
...
_PARAM_NAMES = ['trail_length', 'decay', 'blur_amount', 'zoom', 'rotation', 'hue_drift', 'chromatic_drift', 'freeze_gate']
_PARAM_DEFAULTS = {'trail_length': 5.0, 'decay': 3.0, 'blur_amount': 3.0, 'zoom': 0.0, 'rotation': 0.0, 'hue_drift': 0.0, 'chromatic_drift': 0.0, 'freeze_gate': 0.0}
...
class TrailsPlugin(EffectPlugin):

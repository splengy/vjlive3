"""
P7-VE18: Blend modes — configurable blend mode processor.
...
METADATA = {
    "name": "Blend Modes Effect",
    "description": "Blend modes — configurable blend mode processor.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['blend', 'composite', 'modes', 'layer', 'mix'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'amount', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'invert_b', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'opacity', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float mode;  // Blend mode select
uniform float amount;  // Blend amount
uniform float gamma;  // Gamma
uniform float invert_b;  // Invert layer B
...
_PARAM_NAMES = ['mode', 'amount', 'gamma', 'invert_b', 'opacity', 'mix']
_PARAM_DEFAULTS = {'mode': 0.0, 'amount': 8.0, 'gamma': 5.0, 'invert_b': 0.0, 'opacity': 8.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BlendModePlugin(EffectPlugin):
    """Blend modes — configurable blend mode processor."""
    def __init__(self):
        super().__init__()

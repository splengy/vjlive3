"""
P7-VE47: Luma/chroma mask — HSL-based selection mask.
...
METADATA = {
    "name": "Luma Chroma Mask",
    "description": "Luma/chroma mask — HSL-based selection mask.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['luma', 'chroma', 'mask', 'selection', 'keying'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'luma_min', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'luma_max', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_range', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'feather', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float luma_min;  // Min luminance
uniform float luma_max;  // Max luminance
uniform float chroma_hue;  // Target hue
uniform float chroma_range;  // Hue range
uniform float feather;  // Mask feather
...
void main() {
    ...
    c.r *= 1.+chroma_hue/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['luma_min', 'luma_max', 'chroma_hue', 'chroma_range', 'feather', 'mix']
_PARAM_DEFAULTS = {'luma_min': 0.0, 'luma_max': 10.0, 'chroma_hue': 5.0, 'chroma_range': 3.0, 'feather': 3.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class LumaChromaMaskPlugin(EffectPlugin):
    """Luma/chroma mask — HSL-based selection mask."""
    def __init__(self):
        super().__init__()

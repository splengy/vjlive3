"""
P6-GE07: Oscillator generator — SVG-quality video oscilloscope with FM synthesis.
...
METADATA = {
    "name": "Osc Effect",
    "description": "Oscillator generator — SVG-quality video oscilloscope with FM synthesis.",
    "version": "1.0.0",
    "plugin_type": "generator",
    "category": "generator",
    "tags": ['oscillator', 'waveform', 'lissajous', 'generative', 'synth'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'frequency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sync', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'offset', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'waveform', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'thickness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mirror', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'pulse_width', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mod_depth', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mod_speed', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'osc2_freq', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'output_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float frequency;  // 0-200 Hz
uniform float sync;  // Speed multiplier
uniform float offset;  // Phase offset
uniform float waveform;  // 0=sine,3=tri,6=saw,9=square
uniform float thickness;  // Line/fill thickness
uniform float color_shift;  // Hue rotation
uniform float saturation;  // Saturation
uniform float rotation;  // Pattern rotation
uniform float mirror;  // 0=none,5=vert,10=radial
uniform float pulse_width;  // PWM duty
uniform float mod_depth;  // FM depth
uniform float mod_speed;  // FM rate
uniform float osc2_freq;  // 2nd oscillator
uniform float output_mode;  // Output mode
...
void main() {
    ...
    float disp     = 0.0;
    float feed     = 0.0;
    float freq_v   = 8.0 + (frequency / 10.0) * 10.0;
    float chroma   = 0.0;
    float vign_v   = 0.0;
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + saturation/10.0*0.2;
    color.b *= 1.0 + 0.0;
...
_PARAM_NAMES = ['frequency', 'sync', 'offset', 'waveform', 'thickness', 'color_shift', 'saturation', 'rotation', 'mirror', 'pulse_width', 'mod_depth', 'mod_speed', 'osc2_freq', 'output_mode']
_PARAM_DEFAULTS = {'frequency': 5.0, 'sync': 5.0, 'offset': 0.0, 'waveform': 0.0, 'thickness': 5.0, 'color_shift': 0.0, 'saturation': 5.0, 'rotation': 0.0, 'mirror': 0.0, 'pulse_width': 5.0, 'mod_depth': 0.0, 'mod_speed': 0.0, 'osc2_freq': 0.0, 'output_mode': 0.0}

"""
P7-VE57: Reaction diffusion — Gray-Scott RD simulation.
...
METADATA = {
    ...
    "description": "Reaction diffusion — Gray-Scott RD simulation.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['reaction-diffusion', 'gray-scott', 'simulation', 'organic', 'pattern'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'feed_rate', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'kill_rate', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'diffuse_a', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'diffuse_b', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float feed_rate;  // Feed rate
uniform float kill_rate;  // Kill rate
uniform float diffuse_a;  // Diffuse A
uniform float diffuse_b;  // Diffuse B
uniform float speed;  // Simulation speed
...
_PARAM_NAMES = ['feed_rate', 'kill_rate', 'diffuse_a', 'diffuse_b', 'speed', 'mix']
_PARAM_DEFAULTS = {'feed_rate': 5.0, 'kill_rate': 5.0, 'diffuse_a': 5.0, 'diffuse_b': 5.0, 'speed': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ReactionDiffusionPlugin(EffectPlugin):
    """Reaction diffusion — Gray-Scott RD simulation."""
    def __init__(self):
        super().__init__()

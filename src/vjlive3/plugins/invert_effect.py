"""
P7-VE25: Invert — color and luminance inversion.
...
METADATA = {
    "name": "Invert Effect",
    "description": "Invert — color and luminance inversion.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['invert', 'negative', 'color', 'complement', 'grade'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amount', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'channel_r', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'channel_g', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'channel_b', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'luma_only', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float amount;  // Invert amount
uniform float channel_r;  // Invert red
uniform float channel_g;  // Invert green
uniform float channel_b;  // Invert blue
uniform float luma_only;  // Luma only
...
_PARAM_NAMES = ['amount', 'channel_r', 'channel_g', 'channel_b', 'luma_only', 'mix']
_PARAM_DEFAULTS = {'amount': 10.0, 'channel_r': 10.0, 'channel_g': 10.0, 'channel_b': 10.0, 'luma_only': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class InvertPlugin(EffectPlugin):
    """Invert — color and luminance inversion."""
    def __init__(self):
        super().__init__()

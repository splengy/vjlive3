"""
P5-DM36: Volumetric glitch — depth-layered glitch artifacts in 3D space.
Ported from VJlive-2/plugins/vdatamosh/volumetric_glitch.py.
"""
...
METADATA = {
    "name": "Volumetric Glitch",
    "description": "Volumetric glitch — depth-layered glitch artifacts in 3D space.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['volumetric', 'glitch', 'depth', 'datamosh', '3d'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'trail_intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'volumetric_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'glitch_rate', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'block_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_layers', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_split', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'temporal_chaos', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'artifact_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation_shift', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'vignette', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float trail_intensity;  // Trail intensity
uniform float volumetric_density;  // Volume density
uniform float glitch_rate;  // Glitch rate
uniform float block_size;  // Block size
uniform float depth_layers;  // Depth layers
uniform float chroma_split;  // Chromatic
uniform float temporal_chaos;  // Temporal chaos
uniform float artifact_strength;  // Artifact
uniform float feedback;  // Feedback
uniform float decay;  // Decay
uniform float saturation_shift;  // Saturation
...
void main() {
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + glitch_rate/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['trail_intensity', 'volumetric_density', 'glitch_rate', 'block_size', 'depth_layers', 'chroma_split', 'temporal_chaos', 'artifact_strength', 'feedback', 'decay', 'saturation_shift', 'vignette']
_PARAM_DEFAULTS = {'trail_intensity': 5.0, 'volumetric_density': 5.0, 'glitch_rate': 5.0, 'block_size': 3.0, 'depth_layers': 5.0, 'chroma_split': 2.0, 'temporal_chaos': 5.0, 'artifact_strength': 5.0, 'feedback': 5.0, 'decay': 5.0, 'saturation_shift': 3.0, 'vignette': 3.0}
...
class VolumetricGlitchPlugin(EffectPlugin):

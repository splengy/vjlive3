"""
P5-DM31: Temporal rift — depth zones displaced across time, chaos layering.
Ported from VJlive-2/plugins/vdatamosh/temporal_rift_datamosh.py.
"""
...
METADATA = {
    "name": "Temporal Rift Datamosh",
    "description": "Temporal rift — depth zones displaced across time, chaos layering.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['temporal', 'rift', 'time', 'datamosh', 'psychedelic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'rift_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'time_stretch', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'propagation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'layer_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chaos', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_bleed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'block_res', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'motion_sens', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend_mode', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'temporal_decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'spatial_warp', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float rift_depth;  // Rift depth
uniform float time_stretch;  // Time stretch
uniform float propagation;  // Propagation
uniform float layer_count;  // Layer count
uniform float chaos;  // Chaos
uniform float feedback;  // Feedback
uniform float color_bleed;  // Color bleed
uniform float block_res;  // Block resolution
uniform float motion_sens;  // Motion sensitivity
uniform float blend_mode;  // Blend mode
uniform float temporal_decay;  // Temporal decay
uniform float spatial_warp;  // Spatial warp
...
void main() {
    ...
    float displacement = (rift_depth / 10.0);
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + temporal_decay/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['rift_depth', 'time_stretch', 'propagation', 'layer_count', 'chaos', 'feedback', 'color_bleed', 'block_res', 'motion_sens', 'blend_mode', 'temporal_decay', 'spatial_warp']
_PARAM_DEFAULTS = {'rift_depth': 5.0, 'time_stretch': 5.0, 'propagation': 5.0, 'layer_count': 5.0, 'chaos': 5.0, 'feedback': 5.0, 'color_bleed': 3.0, 'block_res': 5.0, 'motion_sens': 5.0, 'blend_mode': 5.0, 'temporal_decay': 5.0, 'spatial_warp': 5.0}
...
class TemporalRiftDatamoshPlugin(EffectPlugin):

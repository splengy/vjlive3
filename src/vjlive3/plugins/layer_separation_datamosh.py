"""
P5-DM17: Layer separation — splits depth map into fore/mid/background planes.
Ported from VJlive-2/plugins/vdatamosh/layer_separation_datamosh.py.
"""
...
METADATA = {
    "name": "Layer Separation Datamosh",
    "description": "Layer separation — splits depth map into fore/mid/background planes.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['layers', 'depth', 'separation', 'datamosh', 'parallax'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
...
uniform float time;
uniform float u_mix;
uniform float intensity;  // Separation strength
uniform float scale;  // Scale factor
uniform float speed;  // Animation speed
uniform float threshold;  // Layer threshold
uniform float foreground_mix;  // FG layer
uniform float background_mix;  // BG layer
...
class LayerSeparationPlugin(EffectPlugin):

"""
P7-VE79: Vimana synth — synthesizer-driven sacred geometry.
...
METADATA = {
    "name": "Vimana Synth",
    "description": "Vimana synth — synthesizer-driven sacred geometry.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['vimana', 'synth', 'geometry', 'oscillator', 'sacred'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'synth_freq', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'amplitude', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'waveform', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'geometry_type', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float synth_freq;  // Synth frequency
uniform float amplitude;  // Amplitude
uniform float waveform;  // Waveform type
uniform float geometry_type;  // Geometry
...
void main() {
    ...
    float d = (amplitude/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
...
_PARAM_NAMES = ['synth_freq', 'amplitude', 'waveform', 'geometry_type', 'color_hue', 'mix']
_PARAM_DEFAULTS = {'synth_freq': 5.0, 'amplitude': 5.0, 'waveform': 0.0, 'geometry_type': 5.0, 'color_hue': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class VimanaSynthPlugin(EffectPlugin):
    """Vimana synth — synthesizer-driven sacred geometry."""
    def __init__(self):
        super().__init__()

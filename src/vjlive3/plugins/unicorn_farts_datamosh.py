"""
P5-DM33: Unicorn farts — rainbow sparkles, glitter, pastel joy trails.
Ported from VJlive-2/plugins/vdatamosh/unicorn_farts_datamosh.py.
"""
...
METADATA = {
    "name": "Unicorn Farts Datamosh",
    "description": "Unicorn farts — rainbow sparkles, glitter, pastel joy trails.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['rainbow', 'sparkle', 'cute', 'datamosh', 'colorful'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'rainbow_power', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sparkle_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sparkle_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'glitter_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'pastel', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'star_bloom', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'shimmer', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shimmer_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'candy_shift', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mosh_joy', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'trail_sparkle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_rainbow', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float rainbow_power;  // Rainbow intensity
uniform float sparkle_density;  // Sparkle count
uniform float sparkle_speed;  // Sparkle speed
uniform float glitter_size;  // Glitter size
uniform float pastel;  // Pastel amount
uniform float star_bloom;  // Star bloom
uniform float shimmer;  // Shimmer
uniform float shimmer_speed;  // Shimmer speed
uniform float candy_shift;  // Candy hue shift
uniform float mosh_joy;  // Joy datamosh
uniform float trail_sparkle;  // Trail sparkle
uniform float bass_rainbow;  // Bass rainbow
...
void main() {
    ...
    float displacement = (rainbow_power / 10.0);
    float feedback_v   = (bass_rainbow / 10.0);
    float freq         = 8.0 + (sparkle_speed / 10.0) * 10.0;
    float chroma       = 0.0;
    float vign_v       = 0.0;
    ...
    color.r *= 1.0 + rainbow_power/10.0*0.3;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + star_bloom/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['rainbow_power', 'sparkle_density', 'sparkle_speed', 'glitter_size', 'pastel', 'star_bloom', 'shimmer', 'shimmer_speed', 'candy_shift', 'mosh_joy', 'trail_sparkle', 'bass_rainbow']
_PARAM_DEFAULTS = {'rainbow_power': 5.0, 'sparkle_density': 5.0, 'sparkle_speed': 5.0, 'glitter_size': 3.0, 'pastel': 5.0, 'star_bloom': 3.0, 'shimmer': 5.0, 'shimmer_speed': 5.0, 'candy_shift': 5.0, 'mosh_joy': 5.0, 'trail_sparkle': 5.0, 'bass_rainbow': 5.0}
...
class UnicornFartsDatamoshPlugin(EffectPlugin):

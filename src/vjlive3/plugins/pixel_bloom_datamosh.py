"""
P5-DM12: Pixel bloom datamosh — radiant bloom, pixel overgrowth, colorful expansion.
Ported from VJlive-2/plugins/vdatamosh/pixel_bloom_datamosh.py.
"""
...
METADATA = {
    "name": "Pixel Bloom Datamosh",
    "description": "Pixel bloom datamosh — radiant bloom, pixel overgrowth, colorful expansion.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['bloom', 'glow', 'datamosh', 'colorful', 'organic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'bloom_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bloom_radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pixel_size', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'expansion', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation_boost', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pulse_speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'chromatic', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'glow_tint', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float bloom_strength;  // Bloom amount
uniform float bloom_radius;  // Bloom radius
uniform float pixel_size;  // Pixel size
uniform float color_shift;  // Color hue rotation
uniform float expansion;  // Expansion rate
uniform float threshold;  // Bloom threshold
uniform float saturation_boost;  // Saturation boost
uniform float decay;  // Decay rate
uniform float pulse_speed;  // Pulse speed
uniform float chromatic;  // Chromatic amount
uniform float feedback;  // Feedback
uniform float glow_tint;  // Glow tint
...
void main() {
    ...
    float displacement = (bloom_strength / 10.0);
    ...
    color.r *= 1.0 + bloom_strength/10.0*0.3;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + bloom_strength/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['bloom_strength', 'bloom_radius', 'pixel_size', 'color_shift', 'expansion', 'threshold', 'saturation_boost', 'decay', 'pulse_speed', 'chromatic', 'feedback', 'glow_tint']
_PARAM_DEFAULTS = {'bloom_strength': 5.0, 'bloom_radius': 5.0, 'pixel_size': 5.0, 'color_shift': 3.0, 'expansion': 5.0, 'threshold': 5.0, 'saturation_boost': 5.0, 'decay': 5.0, 'pulse_speed': 3.0, 'chromatic': 2.0, 'feedback': 5.0, 'glow_tint': 3.0}
...
class PixelBloomDatamoshPlugin(EffectPlugin):

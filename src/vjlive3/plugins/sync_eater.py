"""
P7-VE66: Sync eater — consumes and desynchronises video sync.
...
METADATA = {
    "name": "Sync Eater",
    "description": "Sync eater — consumes and desynchronises video sync.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['sync', 'glitch', 'desync', 'analog', 'degradation'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'eat_amount', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'horizontal_tear', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'vertical_roll', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'noise', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'tracking', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float eat_amount;  // Sync destruction
uniform float horizontal_tear;  // H-tear
uniform float vertical_roll;  // V-roll
uniform float noise;  // Noise
uniform float tracking;  // Tracking error
...
void main() {
    ...
    float d = (eat_amount/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+0.0; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['eat_amount', 'horizontal_tear', 'vertical_roll', 'noise', 'tracking', 'mix']
_PARAM_DEFAULTS = {'eat_amount': 5.0, 'horizontal_tear': 5.0, 'vertical_roll': 3.0, 'noise': 3.0, 'tracking': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class SyncEaterPlugin(EffectPlugin):
    """Sync eater — consumes and desynchronises video sync."""
    def __init__(self):
        super().__init__()

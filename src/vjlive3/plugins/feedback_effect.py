"""
P7-VE08: Feedback — analog feedback loop simulation.
...
METADATA = {
    ...
    "description": "Feedback — analog feedback loop simulation.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['feedback', 'loop', 'trail', 'temporal', 'analog'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amount', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'zoom', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotate_speed', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_drift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float amount;  // Feedback strength
uniform float decay;  // Brightness decay
uniform float zoom;  // Zoom per frame
uniform float rotate_speed;  // Rotation speed
...
_PARAM_NAMES = ['amount', 'decay', 'zoom', 'rotate_speed', 'hue_drift', 'mix']
_PARAM_DEFAULTS = {'amount': 5.0, 'decay': 3.0, 'zoom': 0.0, 'rotate_speed': 0.0, 'hue_drift': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class FeedbackPlugin(EffectPlugin):
    """Feedback — analog feedback loop simulation."""
    def __init__(self):
        super().__init__()

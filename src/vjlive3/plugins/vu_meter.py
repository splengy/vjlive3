"""P7-VE82: VU meter — audio level VU meter overlay."""
...
METADATA = {
    "name": "Vu Meter",
    "description": "VU meter — audio level VU meter overlay.",
    "version": "1.0.0", "plugin_type": "effect", "category": "visualizer",
    "tags": ["vu", "meter", "audio", "level", "overlay"],
    "priority": 1, "inputs": ["video_in"], "outputs": ["video_out"],
    "parameters": [
        {"name": "peak_hold", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
        {"name": "response", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
        {"name": "color_hue", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
        {"name": "width", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
        {"name": "position", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
...
uniform float peak_hold;uniform float response;uniform float color_hue;
uniform float width;uniform float position;
void main(){fragColor=mix(texture(tex0,uv),texture(prev_tex,uv),u_mix*0.3);}
"""
_PARAM_NAMES = ["peak_hold","response","color_hue","width","position"]
_PARAM_DEFAULTS = {"peak_hold":5.,"response":5.,"color_hue":5.,"width":5.,"position":5.}

def _map(v,lo,hi): return lo+(max(0.,min(10.,float(v)))/10.)*(hi-lo)

class VUMeterPlugin(EffectPlugin):
    def __init__(self):
        super().__init__()

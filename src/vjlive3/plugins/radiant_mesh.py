"""
P6-P305: Radiant mesh — glowing mesh overlay driven by audio adrenaline.
...
METADATA = {
    "name": "Radiant Mesh",
    "description": "Radiant mesh — glowing mesh overlay driven by audio adrenaline.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "particles",
    "tags": ['mesh', 'radiant', 'glow', 'audio', 'neon'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'adrenaline', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mesh_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'glow_radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pulse_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation_speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_bias', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_boost', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'treble_sparkle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'transparency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_glow', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float adrenaline;  // Audio adrenaline drive
uniform float mesh_density;  // Mesh density
uniform float glow_radius;  // Glow radius
uniform float color_hue;  // Base hue
uniform float pulse_speed;  // Pulse speed
uniform float rotation_speed;  // Rotation
uniform float depth_bias;  // Depth bias
uniform float feedback;  // Feedback
uniform float bass_boost;  // Bass boost
uniform float treble_sparkle;  // Treble sparkle
uniform float transparency;  // Mesh transparency
...
void main() {
    ...
    float disp     = (adrenaline / 10.0);
    float feed     = (feedback / 10.0);
    float freq_v   = 8.0 + (pulse_speed / 10.0) * 10.0;
    float chroma   = 0.0;
    float vign_v   = (depth_bias / 10.0);
    ...
    color.r *= 1.0 + adrenaline/10.0*0.3;
    color.g *= 1.0 + glow_radius/10.0*0.2;
...
_PARAM_NAMES = ['adrenaline', 'mesh_density', 'glow_radius', 'color_hue', 'pulse_speed', 'rotation_speed', 'depth_bias', 'feedback', 'bass_boost', 'treble_sparkle', 'transparency', 'edge_glow']
_PARAM_DEFAULTS = {'adrenaline': 5.0, 'mesh_density': 5.0, 'glow_radius': 5.0, 'color_hue': 5.0, 'pulse_speed': 5.0, 'rotation_speed': 3.0, 'depth_bias': 5.0, 'feedback': 3.0, 'bass_boost': 5.0, 'treble_sparkle': 5.0, 'transparency': 5.0, 'edge_glow': 5.0}
...
class RadiantMeshPlugin(EffectPlugin):

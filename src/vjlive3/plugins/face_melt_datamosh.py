"""
P5-DM21: Face melt — viscous face-to-floor melting with skull reveal.
Ported from VJlive-2/plugins/vdatamosh/face_melt_datamosh.py.
"""
...
METADATA = {
    ...
    "description": "Face melt — viscous face-to-floor melting with skull reveal.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['face', 'melt', 'horror', 'datamosh', 'organic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'melt_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'viscosity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'face_isolation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'skull_reveal', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'eye_retention', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_bleed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'turbulence', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'floor_pooling', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'identity_loss', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'scream_echo', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'background_stab', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'nightmare_fuel', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float melt_speed;  // Drip speed
uniform float viscosity;  // Fluid thickness
uniform float face_isolation;  // Face detection zone
uniform float skull_reveal;  // Skeletal reveal
uniform float eye_retention;  // Eye retention
uniform float color_bleed;  // Color bleed
uniform float turbulence;  // Chaos
uniform float floor_pooling;  // Floor pooling
uniform float identity_loss;  // Identity distortion
uniform float scream_echo;  // Echo
uniform float background_stab;  // BG artifact
uniform float nightmare_fuel;  // Horror intensity
...
void main() {
    ...
    float displacement = (melt_speed / 10.0);
    float feedback_v   = (nightmare_fuel / 10.0);
    float freq         = 8.0 + 5.0 * 10.0;
    float chroma       = (color_bleed / 10.0);
    float vign_v       = 0.0;
...
_PARAM_NAMES = ['melt_speed', 'viscosity', 'face_isolation', 'skull_reveal', 'eye_retention', 'color_bleed', 'turbulence', 'floor_pooling', 'identity_loss', 'scream_echo', 'background_stab', 'nightmare_fuel']
_PARAM_DEFAULTS = {'melt_speed': 5.0, 'viscosity': 5.0, 'face_isolation': 5.0, 'skull_reveal': 3.0, 'eye_retention': 5.0, 'color_bleed': 3.0, 'turbulence': 3.0, 'floor_pooling': 3.0, 'identity_loss': 5.0, 'scream_echo': 2.0, 'background_stab': 2.0, 'nightmare_fuel': 3.0}
...
class FaceMeltDatamoshPlugin(EffectPlugin):

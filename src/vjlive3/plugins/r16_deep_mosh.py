"""
P7-VE55: R16 deep mosh studio — extreme datamosh resampler.
...
METADATA = {
    "name": "R16 Deep Mosh",
    "description": "R16 deep mosh studio — extreme datamosh resampler.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['r16', 'deep-mosh', 'extreme', 'datamosh', 'studio'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'block_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'temporal_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_corrupt', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float intensity;  // Mosh intensity
uniform float block_size;  // Block size
uniform float temporal_depth;  // Time depth
uniform float color_corrupt;  // Color corruption
...
void main() {
    ...
    float d = (intensity/10.0); float fb = (feedback/10.0); float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+color_corrupt/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+feedback/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['intensity', 'block_size', 'temporal_depth', 'color_corrupt', 'feedback', 'mix']
_PARAM_DEFAULTS = {'intensity': 5.0, 'block_size': 3.0, 'temporal_depth': 5.0, 'color_corrupt': 3.0, 'feedback': 5.0, 'mix': 8.0}

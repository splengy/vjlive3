"""
P7-VE34: Dithering — ordered/error-diffusion dithering.
...
METADATA = {
    ...
    "description": "Dithering — ordered/error-diffusion dithering.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['dither', 'quantize', 'retro', '8bit', 'stylize'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'levels', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'dither_type', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float levels;  // Bit depth
uniform float dither_type;  // 0=ordered,5=Floyd-Steinberg
uniform float scale;  // Dither matrix scale
uniform float gamma;  // Gamma
uniform float color_mode;  // Color/BW
...
void main() {
    ...
    c.r *= 1.+dither_type/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['levels', 'dither_type', 'scale', 'gamma', 'color_mode', 'mix']
_PARAM_DEFAULTS = {'levels': 5.0, 'dither_type': 0.0, 'scale': 5.0, 'gamma': 5.0, 'color_mode': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class DitheringPlugin(EffectPlugin):
    """Dithering — ordered/error-diffusion dithering."""
    def __init__(self):
        super().__init__()

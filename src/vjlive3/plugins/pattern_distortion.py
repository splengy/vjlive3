"""
P7-VE33: Pattern distortion — tiled-pattern UV warp.
...
METADATA = {
    ...
    "description": "Pattern distortion — tiled-pattern UV warp.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['pattern', 'distortion', 'tile', 'uv', 'warp'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'pattern_type', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float pattern_type;  // Pattern type
...
void main() {
    ...
    float d = (strength/10.0); float fb = 0.0; float fr = 8.0+(speed/10.0)*10.0;
...
_PARAM_NAMES = ['pattern_type', 'scale', 'strength', 'speed', 'rotation', 'mix']
_PARAM_DEFAULTS = {'pattern_type': 0.0, 'scale': 5.0, 'strength': 5.0, 'speed': 3.0, 'rotation': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class PatternDistortionPlugin(EffectPlugin):
    """Pattern distortion — tiled-pattern UV warp."""
    def __init__(self):
        super().__init__()

"""
P7-VE29: Color grade — split-tone colour grading.
...
METADATA = {
    "name": "Color Grade2",
    "description": "Color grade — split-tone colour grading.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['color-grade', 'lut', 'cinematic', 'shadows', 'highlights'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'shadow_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'highlight_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'contrast', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'temperature', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float temperature;  // Color temp
...
void main() {
    ...
    c.r *= 1.+shadow_hue/10.0*0.2; c.g *= 1.+saturation/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['shadow_hue', 'highlight_hue', 'contrast', 'saturation', 'temperature', 'mix']
_PARAM_DEFAULTS = {'shadow_hue': 5.0, 'highlight_hue': 5.0, 'contrast': 5.0, 'saturation': 5.0, 'temperature': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ColorGradePlugin(EffectPlugin):
    """Color grade — split-tone colour grading."""
    def __init__(self):
        super().__init__()

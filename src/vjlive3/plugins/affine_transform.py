"""
P7-VE63: Affine transform — full 2D affine matrix.
...
METADATA = {
    ...
    "description": "Affine transform — full 2D affine matrix.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['affine', 'transform', 'matrix', '2d', 'warp'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'scale_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'scale_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shear_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shear_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'translate_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float scale_x;  // Scale X
uniform float scale_y;  // Scale Y
uniform float shear_x;  // Shear X
uniform float shear_y;  // Shear Y
uniform float translate_x;  // Translate X
...
void main() {
    ...
    c.r *= 1.+shear_x/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['scale_x', 'scale_y', 'shear_x', 'shear_y', 'translate_x', 'mix']
_PARAM_DEFAULTS = {'scale_x': 5.0, 'scale_y': 5.0, 'shear_x': 5.0, 'shear_y': 5.0, 'translate_x': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class AffineTransformPlugin(EffectPlugin):
    """Affine transform — full 2D affine matrix."""
    def __init__(self):
        super().__init__()

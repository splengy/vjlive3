"""
P7-VE31: Displacement map — UV warp from external map.
...
METADATA = {
    ...
    "description": "Displacement map — UV warp from external map.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['displacement', 'uv', 'warp', 'distortion', 'map'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'map_scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'x_axis', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'y_axis', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'time_warp', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float map_scale;  // Map scale
uniform float x_axis;  // X channel weight
uniform float y_axis;  // Y channel weight
uniform float time_warp;  // Animate map
...
_PARAM_NAMES = ['strength', 'map_scale', 'x_axis', 'y_axis', 'time_warp', 'mix']
_PARAM_DEFAULTS = {'strength': 5.0, 'map_scale': 5.0, 'x_axis': 5.0, 'y_axis': 5.0, 'time_warp': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class DisplacementMapPlugin(EffectPlugin):
    """Displacement map — UV warp from external map."""
    def __init__(self):
        super().__init__()

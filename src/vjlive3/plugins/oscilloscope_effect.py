"""
P7-VE51: Oscilloscope — video oscilloscope display.
...
METADATA = {
    "name": "Oscilloscope Effect",
    "description": "Oscilloscope — video oscilloscope display.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['oscilloscope', 'scope', 'waveform', 'audio', 'display'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'zoom', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'trace_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'persistence', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'grid_opacity', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'trigger_level', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float zoom;  // Display zoom
uniform float trace_hue;  // Trace color
uniform float persistence;  // Phosphor persistence
uniform float grid_opacity;  // Grid
uniform float trigger_level;  // Trigger level
...
void main() {
    ...
    float d = (zoom/10.0); float fb = (persistence/10.0); float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+trace_hue/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+zoom/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['zoom', 'trace_hue', 'persistence', 'grid_opacity', 'trigger_level', 'mix']
_PARAM_DEFAULTS = {'zoom': 5.0, 'trace_hue': 5.0, 'persistence': 5.0, 'grid_opacity': 3.0, 'trigger_level': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class OscilloscopePlugin(EffectPlugin):
    """Oscilloscope — video oscilloscope display."""
    def __init__(self):
        super().__init__()

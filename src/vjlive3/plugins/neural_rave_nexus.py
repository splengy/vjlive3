"""
P6-QC11: Neural rave nexus — AI-driven synesthetic rave pattern generator.
...
METADATA = {
    "name": "Neural Rave Nexus",
    "description": "Neural rave nexus — AI-driven synesthetic rave pattern generator.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['neural', 'rave', 'synesthetic', 'psychedelic', 'generative'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'nexus_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pulse_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'connection_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_cycle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_response', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'treble_sparkle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chaos_factor', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float nexus_density;  // Node density
uniform float pulse_speed;  // Pulse speed
uniform float connection_strength;  // Node connections
uniform float color_cycle;  // Color cycle
uniform float bass_response;  // Bass response
uniform float treble_sparkle;  // Treble sparkle
uniform float feedback_depth;  // Feedback
uniform float chaos_factor;  // Chaos
...
void main() {
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + feedback_depth/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['nexus_density', 'pulse_speed', 'connection_strength', 'color_cycle', 'bass_response', 'treble_sparkle', 'feedback_depth', 'chaos_factor']
_PARAM_DEFAULTS = {'nexus_density': 5.0, 'pulse_speed': 5.0, 'connection_strength': 5.0, 'color_cycle': 5.0, 'bass_response': 5.0, 'treble_sparkle': 5.0, 'feedback_depth': 5.0, 'chaos_factor': 3.0}
...
class NeuralRaveNexusPlugin(EffectPlugin):

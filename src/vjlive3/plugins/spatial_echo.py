"""
P7-VE74: Spatial echo — directional delay echo cascade.
...
METADATA = {
    "name": "Spatial Echo",
    "description": "Spatial echo — directional delay echo cascade.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['echo', 'spatial', 'delay', 'cascade', 'feedback'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'echo_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'echo_delay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'echo_decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'direction', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float echo_count;  // Echo count
uniform float echo_delay;  // Delay per echo
uniform float echo_decay;  // Echo fade
uniform float direction;  // Echo direction
uniform float color_shift;  // Hue per echo
...
void main() {
    ...
    float d = 0.0; float fb = (echo_decay/10.0); float fr = 8.0+(echo_count/10.0)*10.0;
    ...
    c.r *= 1.+color_shift/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['echo_count', 'echo_delay', 'echo_decay', 'direction', 'color_shift', 'mix']
_PARAM_DEFAULTS = {'echo_count': 5.0, 'echo_delay': 5.0, 'echo_decay': 5.0, 'direction': 5.0, 'color_shift': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class SpatialEchoPlugin(EffectPlugin):
    """Spatial echo — directional delay echo cascade."""
    def __init__(self):
        super().__init__()

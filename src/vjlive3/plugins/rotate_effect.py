"""
P7-VE37: Rotate — continuous rotation transform.
...
METADATA = {
    "name": "Rotate Effect",
    "description": "Rotate — continuous rotation transform.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['rotate', 'transform', 'geometry', 'angle', 'spin'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'angle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'zoom', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float speed;  // Auto-rotate speed
uniform float center_x;  // Pivot X
uniform float center_y;  // Pivot Y
uniform float zoom;  // Scale during rotate
...
void main() {
    ...
    float d = (zoom/10.0); float fb = 0.0; float fr = 8.0+(speed/10.0)*10.0;
    ...
    c.r *= 1.+center_x/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+zoom/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['angle', 'speed', 'center_x', 'center_y', 'zoom', 'mix']
_PARAM_DEFAULTS = {'angle': 5.0, 'speed': 0.0, 'center_x': 5.0, 'center_y': 5.0, 'zoom': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class RotatePlugin(EffectPlugin):
    """Rotate — continuous rotation transform."""
    def __init__(self):
        super().__init__()

"""
P5-DM15: Frame hold datamosh — freeze, stutter, and ghost previous frames.
Ported from VJlive-2/plugins/vdatamosh/frame_hold_datamosh.py.
"""
...
METADATA = {
    "name": "Frame Hold Datamosh",
    "description": "Frame hold datamosh — freeze, stutter, and ghost previous frames.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['framestutter', 'freeze', 'temporal', 'datamosh', 'ghost'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'hold_chance', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'hold_duration', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'ghost_blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'flicker_rate', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chromatic_ghost', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'motion_trigger', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'stutter_depth', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'temporal_echo', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'artifact_strength', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'scan_noise', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend_mode', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float hold_chance;  // Freeze probability
uniform float hold_duration;  // Freeze duration
uniform float ghost_blend;  // Ghost amount
uniform float flicker_rate;  // Flicker speed
uniform float decay;  // Frame decay
uniform float chromatic_ghost;  // Chroma ghost
uniform float motion_trigger;  // Motion trigger
uniform float stutter_depth;  // Stutter depth
uniform float temporal_echo;  // Temporal echo
uniform float artifact_strength;  // Codec artifact
uniform float scan_noise;  // Scan noise
uniform float blend_mode;  // Blend mode
...
void main() {
    ...
    float displacement = (hold_chance / 10.0);
    float feedback_v   = (ghost_blend / 10.0);
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + temporal_echo/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['hold_chance', 'hold_duration', 'ghost_blend', 'flicker_rate', 'decay', 'chromatic_ghost', 'motion_trigger', 'stutter_depth', 'temporal_echo', 'artifact_strength', 'scan_noise', 'blend_mode']
_PARAM_DEFAULTS = {'hold_chance': 3.0, 'hold_duration': 5.0, 'ghost_blend': 5.0, 'flicker_rate': 3.0, 'decay': 5.0, 'chromatic_ghost': 2.0, 'motion_trigger': 5.0, 'stutter_depth': 3.0, 'temporal_echo': 5.0, 'artifact_strength': 3.0, 'scan_noise': 2.0, 'blend_mode': 5.0}
...
class FrameHoldDatamoshPlugin(EffectPlugin):

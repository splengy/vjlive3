"""
P7-VE43: Projection mapping — warp video to flat surface.
...
METADATA = {
    "name": "Projection Mapping2",
    "description": "Projection mapping — warp video to flat surface.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['projection', 'mapping', 'geometry', 'transform', 'installation'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'quad_p1x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'quad_p1y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'warp_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'warp_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'perspective', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float quad_p1x;  // Corner 1 X
uniform float quad_p1y;  // Corner 1 Y
uniform float warp_x;  // Warp X
uniform float warp_y;  // Warp Y
uniform float perspective;  // Perspective
...
_PARAM_NAMES = ['quad_p1x', 'quad_p1y', 'warp_x', 'warp_y', 'perspective', 'mix']
_PARAM_DEFAULTS = {'quad_p1x': 5.0, 'quad_p1y': 5.0, 'warp_x': 5.0, 'warp_y': 5.0, 'perspective': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ProjectionMappingPlugin(EffectPlugin):
    """Projection mapping — warp video to flat surface."""
    def __init__(self):
        super().__init__()

"""
P5-DM20: Dolly zoom datamosh — Hitchcock/Vertigo effect with depth-locked subject.
Ported from VJlive-2/plugins/vdatamosh/dolly_zoom_datamosh.py.
"""
...
METADATA = {
    ...
    "description": "Dolly zoom datamosh — Hitchcock/Vertigo effect with depth-locked subject.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['dolly-zoom', 'vertigo', 'cinematic', 'datamosh', 'depth'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'zoom_intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'subject_dist', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'breath_speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'lens_distort', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'film_grain', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'chromatic_ab', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'dizzy_spin', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'vignette', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'focal_instability', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_cut', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'motion_trails', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'panic_zoom', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float zoom_intensity;  // Zoom strength
uniform float subject_dist;  // Subject depth
uniform float breath_speed;  // Zoom cycle speed
uniform float lens_distort;  // Barrel distort
uniform float film_grain;  // Grain
uniform float chromatic_ab;  // Aberration
uniform float dizzy_spin;  // Rotation
uniform float vignette;  // Corner vignette
uniform float focal_instability;  // Focus jitter
uniform float depth_cut;  // Subject isolation
uniform float motion_trails;  // Trails
uniform float panic_zoom;  // Snap zooms
...
void main() {
    ...
    float displacement = (zoom_intensity / 10.0);
    float feedback_v   = (panic_zoom / 10.0);
    float freq         = 8.0 + (chromatic_ab / 10.0) * 10.0;
    float chroma       = (chromatic_ab / 10.0);
...
_PARAM_NAMES = ['zoom_intensity', 'subject_dist', 'breath_speed', 'lens_distort', 'film_grain', 'chromatic_ab', 'dizzy_spin', 'vignette', 'focal_instability', 'depth_cut', 'motion_trails', 'panic_zoom']
_PARAM_DEFAULTS = {'zoom_intensity': 5.0, 'subject_dist': 5.0, 'breath_speed': 3.0, 'lens_distort': 3.0, 'film_grain': 2.0, 'chromatic_ab': 2.0, 'dizzy_spin': 2.0, 'vignette': 3.0, 'focal_instability': 3.0, 'depth_cut': 5.0, 'motion_trails': 3.0, 'panic_zoom': 3.0}
...
class DollyZoomDatamoshPlugin(EffectPlugin):

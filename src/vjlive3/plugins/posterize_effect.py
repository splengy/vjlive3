"""
P7-VE20: Posterize — reduces to N color levels.
...
METADATA = {
    ...
    "description": "Posterize — reduces to N color levels.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['posterize', 'levels', 'stylize', 'graphic', 'flat'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'levels', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'dither', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_detect', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float levels;  // Color levels
uniform float dither;  // Dither type
uniform float gamma;  // Gamma pre-correction
uniform float saturation;  // Saturation boost
uniform float edge_detect;  // Edge outline
...
_PARAM_NAMES = ['levels', 'dither', 'gamma', 'saturation', 'edge_detect', 'mix']
_PARAM_DEFAULTS = {'levels': 5.0, 'dither': 0.0, 'gamma': 5.0, 'saturation': 5.0, 'edge_detect': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class PosterizePlugin(EffectPlugin):
    """Posterize — reduces to N color levels."""
    def __init__(self):
        super().__init__()

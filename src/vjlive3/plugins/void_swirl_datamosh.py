"""
P5-DM34: Void swirl — black hole spiral, shadow crawl, kaleidoscope darkness.
Ported from VJlive-2/plugins/vdatamosh/void_swirl_datamosh.py.
"""
...
METADATA = {
    "name": "Void Swirl Datamosh",
    "description": "Void swirl — black hole spiral, shadow crawl, kaleidoscope darkness.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['void', 'dark', 'swirl', 'datamosh', 'horror'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'void_crush', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'void_aperture', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shadow_crawl', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'dark_fog', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'spiral_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'spiral_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'kaleidoscope', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'breathing', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'rainbow', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'mosh_decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_drain', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float void_crush;  // Dark crush
uniform float void_aperture;  // Black hole size
uniform float shadow_crawl;  // Shadow movement
uniform float dark_fog;  // Fog density
uniform float spiral_speed;  // Spiral speed
uniform float spiral_strength;  // Spiral force
uniform float kaleidoscope;  // Kaleidoscope
uniform float breathing;  // Breathing
uniform float rainbow;  // Void rainbow
uniform float mosh_decay;  // Mosh decay
uniform float color_drain;  // Color drain
...
void main() {
    ...
    float displacement = (void_crush / 10.0);
    float feedback_v   = (feedback / 10.0);
    float freq         = 8.0 + (spiral_speed / 10.0) * 10.0;
    float chroma       = 0.0;
    float vign_v       = (void_crush / 10.0);
    ...
    color.r *= 1.0 + rainbow/10.0*0.3;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + void_crush/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['void_crush', 'void_aperture', 'shadow_crawl', 'dark_fog', 'spiral_speed', 'spiral_strength', 'kaleidoscope', 'breathing', 'rainbow', 'mosh_decay', 'color_drain', 'feedback']
_PARAM_DEFAULTS = {'void_crush': 5.0, 'void_aperture': 5.0, 'shadow_crawl': 3.0, 'dark_fog': 3.0, 'spiral_speed': 5.0, 'spiral_strength': 5.0, 'kaleidoscope': 3.0, 'breathing': 3.0, 'rainbow': 2.0, 'mosh_decay': 5.0, 'color_drain': 5.0, 'feedback': 5.0}
...
class VoidSwirlDatamoshPlugin(EffectPlugin):

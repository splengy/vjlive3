"""
P7-VE69: Multiband color — frequency-domain color processing.
...
METADATA = {
    "name": "Multiband Color",
    "description": "Multiband color — frequency-domain color processing.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['multiband', 'color', 'frequency', 'hsl', 'grade'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'shadow_sat', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mid_sat', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'highlight_sat', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shadow_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'highlight_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float shadow_sat;  // Shadow saturation
uniform float mid_sat;  // Midtone saturation
uniform float highlight_sat;  // Highlight saturation
uniform float shadow_hue;  // Shadow hue
uniform float highlight_hue;  // Highlight hue
...
void main() {
    ...
    float d = 0.0; float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+shadow_hue/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['shadow_sat', 'mid_sat', 'highlight_sat', 'shadow_hue', 'highlight_hue', 'mix']
_PARAM_DEFAULTS = {'shadow_sat': 5.0, 'mid_sat': 5.0, 'highlight_sat': 5.0, 'shadow_hue': 5.0, 'highlight_hue': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class MultibandColorPlugin(EffectPlugin):
    """Multiband color — frequency-domain color processing."""
    def __init__(self):
        super().__init__()

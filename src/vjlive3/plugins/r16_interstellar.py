"""
P7-VE56: R16 interstellar mosh — space-travel datamosh.
...
METADATA = {
    "name": "R16 Interstellar",
    "description": "R16 interstellar mosh — space-travel datamosh.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['r16', 'interstellar', 'space', 'datamosh', 'cosmic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'warp_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'star_intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mosh_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_shift', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float warp_speed;  // Warp speed
uniform float star_intensity;  // Star field
uniform float mosh_depth;  // Mosh depth
uniform float chroma_shift;  // Chromatic
...
void main() {
    ...
    float d = 0.0; float fb = (feedback/10.0); float fr = 8.0+0.0*10.0;
    float ch = (chroma_shift/10.0); float vg = 0.0;
    ...
    c.r *= 1.+star_intensity/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+feedback/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['warp_speed', 'star_intensity', 'mosh_depth', 'chroma_shift', 'feedback', 'mix']
_PARAM_DEFAULTS = {'warp_speed': 5.0, 'star_intensity': 5.0, 'mosh_depth': 5.0, 'chroma_shift': 3.0, 'feedback': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class R16InterstellarPlugin(EffectPlugin):
    """R16 interstellar mosh — space-travel datamosh."""
    def __init__(self):
        super().__init__()

"""
P7-VE06: ASCII art — quantizes video to ASCII character grid.
...
METADATA = {
    "name": "Ascii Effect2",
    "description": "ASCII art — quantizes video to ASCII character grid.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['ascii', 'text', 'quantize', 'stylize', 'retro'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'char_size', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'brightness_boost', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'contrast', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float char_size;  // Character cell size
uniform float brightness_boost;  // Brightness
uniform float contrast;  // Contrast
uniform float color_mode;  // 0=mono,5=color,10=ansi
uniform float density;  // Character density
...
void main() {
    ...
    c.r *= 1.+char_size/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['char_size', 'brightness_boost', 'contrast', 'color_mode', 'density', 'mix']
_PARAM_DEFAULTS = {'char_size': 5.0, 'brightness_boost': 5.0, 'contrast': 5.0, 'color_mode': 0.0, 'density': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ASCIIPlugin(EffectPlugin):
    """ASCII art — quantizes video to ASCII character grid."""
    def __init__(self):
        super().__init__()

"""
P7-VE23: Hue shift — rotates hue by a set amount.
...
METADATA = {
    "name": "Hue Effect",
    "description": "Hue shift — rotates hue by a set amount.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['hue', 'rotate', 'color', 'shift', 'grade'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'hue_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_range', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'range_center', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feather', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float hue_range;  // Selective range
uniform float range_center;  // Range center
uniform float feather;  // Selection feather
...
_PARAM_NAMES = ['hue_shift', 'saturation', 'hue_range', 'range_center', 'feather', 'mix']
_PARAM_DEFAULTS = {'hue_shift': 0.0, 'saturation': 5.0, 'hue_range': 10.0, 'range_center': 5.0, 'feather': 3.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class HuePlugin(EffectPlugin):
    """Hue shift — rotates hue by a set amount."""
    def __init__(self):
        super().__init__()

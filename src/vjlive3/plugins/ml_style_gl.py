"""
P6-QC09: ML style transfer — GPU-accelerated neural style transfer effect.
...
METADATA = {
    "name": "Ml Style Gl",
    "description": "ML style transfer — GPU-accelerated neural style transfer effect.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['style-transfer', 'neural', 'ml', 'gpu', 'artistic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'style_strength', 'type': 'float', 'default': 7.0, 'min': 0.0, 'max': 10.0}, {'name': 'content_weight', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'style_scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_preserve', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_emphasis', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'texture_blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation_boost', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sharpness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float style_strength;  // Style intensity
uniform float content_weight;  // Content weight
uniform float style_scale;  // Style scale
uniform float color_preserve;  // Color preserve
uniform float edge_emphasis;  // Edge emphasis
uniform float texture_blend;  // Texture blend
uniform float saturation_boost;  // Saturation
...
void main() {
    ...
    float disp     = (style_strength / 10.0);
    float feed     = 0.0;
    float freq_v   = 8.0 + 0.0 * 10.0;
    float chroma   = (edge_emphasis / 10.0);
    float vign_v   = 0.0;
    ...
    color.r *= 1.0 + style_strength/10.0*0.3;
    color.g *= 1.0 + saturation_boost/10.0*0.2;
    color.b *= 1.0 + 0.0;
...
_PARAM_NAMES = ['style_strength', 'content_weight', 'style_scale', 'color_preserve', 'edge_emphasis', 'texture_blend', 'saturation_boost', 'sharpness']
_PARAM_DEFAULTS = {'style_strength': 7.0, 'content_weight': 5.0, 'style_scale': 5.0, 'color_preserve': 5.0, 'edge_emphasis': 3.0, 'texture_blend': 5.0, 'saturation_boost': 5.0, 'sharpness': 5.0}
...
class MLStylePlugin(EffectPlugin):

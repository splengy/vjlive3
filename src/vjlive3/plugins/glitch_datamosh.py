"""
P5-DM05: Digital glitch — block displacement, channel swap, frame corruption.
Ported from VJlive-2/plugins/vdatamosh/glitch_datamosh.py.
"""
...
METADATA = {
    ...
    "description": "Digital glitch — block displacement, channel swap, frame corruption.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['glitch', 'digital', 'datamosh', 'chaotic', 'destructive'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amount', 'type': 'float', 'default': 1.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'block_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'vertical', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'frame_drop', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'channel_swap', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'scanline_jitter', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'corruption', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hold_duration', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float amount;  // Glitch probability
uniform float speed;  // Rate of change
uniform float block_size;  // Block size
uniform float color_shift;  // Chromatic separation
uniform float vertical;  // Vertical shift
uniform float frame_drop;  // Frame drop chance
uniform float channel_swap;  // Channel swap
uniform float scanline_jitter;  // Per-scanline jitter
uniform float corruption;  // Pixel corruption
uniform float hold_duration;  // Hold frames
...
void main() {
    ...
    float displacement = (amount / 10.0);
    float feedback     = (hold_duration / 10.0);

    vec2 du = vec2(
        noise(uv * (8.0 + (block_size / 10.0) * 10.0) + vec2(time * 0.5)) - 0.5,
        noise(uv * (8.0 + (block_size / 10.0) * 10.0) + vec2(1.3, time * 0.4)) - 0.5
    ) * displacement * 0.1;
...
_PARAM_NAMES = ['amount', 'speed', 'block_size', 'color_shift', 'vertical', 'frame_drop', 'channel_swap', 'scanline_jitter', 'corruption', 'hold_duration']
_PARAM_DEFAULTS = {'amount': 1.0, 'speed': 3.0, 'block_size': 3.0, 'color_shift': 2.0, 'vertical': 0.0, 'frame_drop': 0.0, 'channel_swap': 0.0, 'scanline_jitter': 0.0, 'corruption': 0.0, 'hold_duration': 0.0}
...
class GlitchDatamoshPlugin(EffectPlugin):

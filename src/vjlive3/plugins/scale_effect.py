"""
P7-VE38: Scale — scale/zoom transform.
...
METADATA = {
    "name": "Scale Effect",
    "description": "Scale — scale/zoom transform.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['scale', 'zoom', 'transform', 'geometry', 'resize'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'scale_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'scale_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'maintain_aspect', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float scale_x;  // X scale
uniform float scale_y;  // Y scale
uniform float center_x;  // Pivot X
uniform float center_y;  // Pivot Y
uniform float maintain_aspect;  // Aspect ratio lock
...
_PARAM_NAMES = ['scale_x', 'scale_y', 'center_x', 'center_y', 'maintain_aspect', 'mix']
_PARAM_DEFAULTS = {'scale_x': 5.0, 'scale_y': 5.0, 'center_x': 5.0, 'center_y': 5.0, 'maintain_aspect': 8.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ScalePlugin(EffectPlugin):
    """Scale — scale/zoom transform."""
    def __init__(self):
        super().__init__()

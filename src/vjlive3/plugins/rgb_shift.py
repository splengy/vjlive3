"""
P7-VE27: RGB shift — chromatic aberration/RGB separation.
...
METADATA = {
    "name": "Rgb Shift",
    "description": "RGB shift — chromatic aberration/RGB separation.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['rgb', 'chromatic', 'aberration', 'shift', 'glitch'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'shift_r', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shift_g', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shift_b', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'angle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'distance', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float shift_r;  // Red offset
uniform float shift_g;  // Green offset
uniform float shift_b;  // Blue offset
uniform float angle;  // Shift angle
uniform float distance;  // Shift distance
...
void main() {
    ...
    float d = (shift_r/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    float ch = (shift_b/10.0); float vg = 0.0;
...
_PARAM_NAMES = ['shift_r', 'shift_g', 'shift_b', 'angle', 'distance', 'mix']
_PARAM_DEFAULTS = {'shift_r': 5.0, 'shift_g': 5.0, 'shift_b': 5.0, 'angle': 5.0, 'distance': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class RGBShiftPlugin(EffectPlugin):
    """RGB shift — chromatic aberration/RGB separation."""
    def __init__(self):
        super().__init__()

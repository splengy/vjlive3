"""
P7-VE71: Solarize — Man Ray-style partial inversion.
...
METADATA = {
    ...
    "description": "Solarize — Man Ray-style partial inversion.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['solarize', 'man-ray', 'photo', 'invert', 'artistic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'amount', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_mode', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_invert', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float threshold;  // Solarize threshold
uniform float amount;  // Effect strength
uniform float color_mode;  // Color/mono
uniform float hue_invert;  // Hue inversion
...
void main() {
    ...
    c.r *= 1.+color_mode/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['threshold', 'amount', 'color_mode', 'hue_invert', 'gamma', 'mix']
_PARAM_DEFAULTS = {'threshold': 5.0, 'amount': 8.0, 'color_mode': 5.0, 'hue_invert': 0.0, 'gamma': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class SolarizePlugin(EffectPlugin):
    """Solarize — Man Ray-style partial inversion."""
    def __init__(self):
        super().__init__()

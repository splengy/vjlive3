"""
P7-VE16: Bloom shadertoy — shadertoy-style GPU bloom.
...
METADATA = {
    "name": "Bloom Shadertoy",
    "description": "Bloom shadertoy — shadertoy-style GPU bloom.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['bloom', 'shadertoy', 'gpu', 'glow', 'luminance'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'iterations', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float radius;  // Bloom radius
uniform float strength;  // Intensity
uniform float iterations;  // Blur passes
uniform float saturation;  // Bloom color
...
void main() {
    ...
    float d = (strength/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+0.0; c.g *= 1.+saturation/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['threshold', 'radius', 'strength', 'iterations', 'saturation', 'mix']
_PARAM_DEFAULTS = {'threshold': 5.0, 'radius': 5.0, 'strength': 5.0, 'iterations': 5.0, 'saturation': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BloomShadertoyPlugin(EffectPlugin):
    """Bloom shadertoy — shadertoy-style GPU bloom."""
    def __init__(self):
        super().__init__()

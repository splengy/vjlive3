"""
P7-VE46: Living fractal consciousness — organic recursive growth.
...
METADATA = {
    "name": "Living Fractal",
    "description": "Living fractal consciousness — organic recursive growth.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['fractal', 'consciousness', 'living', 'organic', 'recursive'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'growth_rate', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'iteration_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_cycle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'consciousness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float growth_rate;  // Growth rate
uniform float iteration_depth;  // Iterations
uniform float color_cycle;  // Color cycling
uniform float scale;  // Scale
uniform float consciousness;  // Awareness level
...
void main() {
    ...
    c.r *= 1.+color_cycle/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['growth_rate', 'iteration_depth', 'color_cycle', 'scale', 'consciousness', 'mix']
_PARAM_DEFAULTS = {'growth_rate': 5.0, 'iteration_depth': 5.0, 'color_cycle': 5.0, 'scale': 5.0, 'consciousness': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class LivingFractalPlugin(EffectPlugin):
    """Living fractal consciousness — organic recursive growth."""
    def __init__(self):
        super().__init__()

"""
P7-VE12: Scanlines — CRT scanline overlay effect.
...
METADATA = {
    ...
    "description": "Scanlines — CRT scanline overlay effect.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['scanlines', 'crt', 'retro', 'overlay', 'tv'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'curvature', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'flicker', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'glow', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float density;  // Scanline count
uniform float intensity;  // Line darkness
uniform float curvature;  // CRT barrel
uniform float flicker;  // Flicker rate
uniform float glow;  // Phosphor glow
...
_PARAM_NAMES = ['density', 'intensity', 'curvature', 'flicker', 'glow', 'mix']
_PARAM_DEFAULTS = {'density': 5.0, 'intensity': 5.0, 'curvature': 0.0, 'flicker': 0.0, 'glow': 3.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ScanlinesPlugin(EffectPlugin):

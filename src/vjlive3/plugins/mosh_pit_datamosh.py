"""
P5-DM24: Mosh pit — concert crowd compression, slam, sweat, chaos.
Ported from VJlive-2/plugins/vdatamosh/mosh_pit_datamosh.py.
"""
...
METADATA = {
    "name": "Mosh Pit Datamosh",
    "description": "Mosh pit — concert crowd compression, slam, sweat, chaos.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['mosh', 'concert', 'aggressive', 'datamosh', 'crowd'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'slam_intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'compression', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sweat_blur', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'gasp_for_air', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'bruise_color', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'collision_rate', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'crowd_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'panic_attack', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'shutter_speed', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'grit', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'blood_rush', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'no_escape', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float slam_intensity;  // Slam force
uniform float compression;  // Crowd compression
uniform float sweat_blur;  // Sweat blur
uniform float gasp_for_air;  // Air distortion
uniform float bruise_color;  // Bruise tint
uniform float collision_rate;  // Impact rate
uniform float crowd_density;  // Crowd density
uniform float panic_attack;  // Panic
uniform float shutter_speed;  // Motion blur
uniform float grit;  // Grit level
uniform float blood_rush;  // Rush intensity
uniform float no_escape;  // Claustrophobia
...
void main() {
    ...
    float displacement = (slam_intensity / 10.0);
    float feedback_v   = (no_escape / 10.0);
    ...
    color.r *= 1.0 + bruise_color/10.0*0.3;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + 0.0;
...
_PARAM_NAMES = ['slam_intensity', 'compression', 'sweat_blur', 'gasp_for_air', 'bruise_color', 'collision_rate', 'crowd_density', 'panic_attack', 'shutter_speed', 'grit', 'blood_rush', 'no_escape']
_PARAM_DEFAULTS = {'slam_intensity': 5.0, 'compression': 5.0, 'sweat_blur': 3.0, 'gasp_for_air': 3.0, 'bruise_color': 3.0, 'collision_rate': 5.0, 'crowd_density': 5.0, 'panic_attack': 3.0, 'shutter_speed': 3.0, 'grit': 5.0, 'blood_rush': 3.0, 'no_escape': 3.0}
...
class MoshPitDatamoshPlugin(EffectPlugin):

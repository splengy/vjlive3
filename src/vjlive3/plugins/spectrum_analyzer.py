"""P7-VE81: Spectrum analyzer — FFT bar graph audio visualizer."""
from typing import Dict, Any
import logging
try:
    import OpenGL.GL as gl; HAS_GL = True
except ImportError:
    HAS_GL = False
...
METADATA = {
    ...
    "description": "Spectrum analyzer — FFT bar graph audio visualizer.",
    "version": "1.0.0", "plugin_type": "effect", "category": "visualizer",
    "tags": ["spectrum", "fft", "bars", "audio", "visualizer"],
    "priority": 1, "inputs": ["video_in"], "outputs": ["video_out"],
    "parameters": [
        {"name": "freq_min", "type": "float", "default": 0.0, "min": 0.0, "max": 10.0},
        {"name": "freq_max", "type": "float", "default": 10.0, "min": 0.0, "max": 10.0},
        {"name": "bar_count", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
        {"name": "bar_width", "type": "float", "default": 5.0, "min": 0.0, "max": 10.0},
...
uniform float freq_min;uniform float freq_max;uniform float bar_count;
uniform float bar_width;uniform float color_hue;
void main(){fragColor=mix(texture(tex0,uv),texture(prev_tex,uv),u_mix*0.3);}
"""
_PARAM_NAMES = ["freq_min","freq_max","bar_count","bar_width","color_hue"]
_PARAM_DEFAULTS = {"freq_min":0.,"freq_max":10.,"bar_count":5.,"bar_width":5.,"color_hue":5.}

def _map(v,lo,hi): return lo+(max(0.,min(10.,float(v)))/10.)*(hi-lo)

class SpectrumAnalyzerPlugin(EffectPlugin):
    def __init__(self):
        super().__init__()
    ...
    def _make_fbo(self,w,h):
        ...
        for k,v2 in [(gl.GL_TEXTURE_MIN_FILTER,gl.GL_LINEAR),(gl.GL_TEXTURE_MAG_FILTER,gl.GL_LINEAR),(gl.GL_TEXTURE_WRAP_S,gl.GL_CLAMP_TO_EDGE),(gl.GL_TEXTURE_WRAP_T,gl.GL_CLAMP_TO_EDGE)]:

"""
P7-VE28: Color correction — full ASC CDL colour correct.
...
METADATA = {
    "name": "Color Correct",
    "description": "Color correction — full ASC CDL colour correct.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['color-correct', 'grade', 'cdl', 'asc', 'cinematic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'lift_r', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'lift_g', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'lift_b', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gain', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float lift_r;  // Lift R
uniform float lift_g;  // Lift G
uniform float lift_b;  // Lift B
uniform float gamma;  // Gamma
uniform float gain;  // Gain
...
void main() {
    ...
    c.r *= 1.+0.0; c.g *= 1.+gain/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['lift_r', 'lift_g', 'lift_b', 'gamma', 'gain', 'saturation']
_PARAM_DEFAULTS = {'lift_r': 5.0, 'lift_g': 5.0, 'lift_b': 5.0, 'gamma': 5.0, 'gain': 5.0, 'saturation': 5.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ColorCorrectPlugin(EffectPlugin):
    """Color correction — full ASC CDL colour correct."""
    def __init__(self):
        super().__init__()

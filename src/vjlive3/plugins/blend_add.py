"""
P7-VE09: Additive blend — adds two video inputs together.
...
METADATA = {
    "name": "Blend Add",
    "description": "Additive blend — adds two video inputs together.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['blend', 'add', 'composite', 'mix', 'additive'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amount', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'brightness_a', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'brightness_b', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'clamp', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'exposure', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float brightness_a;  // Input A brightness
uniform float brightness_b;  // Input B brightness
uniform float clamp;  // Clamp output
...
_PARAM_NAMES = ['amount', 'brightness_a', 'brightness_b', 'clamp', 'exposure', 'mix']
_PARAM_DEFAULTS = {'amount': 8.0, 'brightness_a': 5.0, 'brightness_b': 5.0, 'clamp': 8.0, 'exposure': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BlendAddPlugin(EffectPlugin):
    """Additive blend — adds two video inputs together."""
    def __init__(self):
        super().__init__()

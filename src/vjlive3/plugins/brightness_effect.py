"""
P7-VE24: Brightness — simple brightness/gain control.
...
METADATA = {
    ...
    "description": "Brightness — simple brightness/gain control.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['brightness', 'gain', 'exposure', 'grade', 'color'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'brightness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gain', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'lift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_temp', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float lift;  // Shadow lift
uniform float gamma;  // Gamma
uniform float color_temp;  // Color temperature
...
void main() {
    ...
    c.r *= 1.+color_temp/10.0*0.2; c.g *= 1.+gain/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['brightness', 'gain', 'lift', 'gamma', 'color_temp', 'mix']
_PARAM_DEFAULTS = {'brightness': 5.0, 'gain': 5.0, 'lift': 0.0, 'gamma': 5.0, 'color_temp': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BrightnessPlugin(EffectPlugin):
    """Brightness — simple brightness/gain control."""
    def __init__(self):
        super().__init__()

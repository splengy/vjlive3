"""
P7-VE36: Mandalascope — kaleidoscopic mandala from video.
...
METADATA = {
    "name": "Mandalascope",
    "description": "Mandalascope — kaleidoscopic mandala from video.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['mandala', 'kaleidoscope', 'symmetry', 'mirror', 'sacred'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'segments', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'zoom', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float segments;  // Mirror segments
uniform float rotation;  // Rotation speed
uniform float zoom;  // Zoom
uniform float hue_shift;  // Hue shift
...
void main() {
    ...
    float d = (zoom/10.0); float fb = (feedback/10.0); float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+hue_shift/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+zoom/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['segments', 'rotation', 'zoom', 'hue_shift', 'feedback', 'mix']
_PARAM_DEFAULTS = {'segments': 5.0, 'rotation': 0.0, 'zoom': 5.0, 'hue_shift': 0.0, 'feedback': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class MandalascopePlugin(EffectPlugin):
    """Mandalascope — kaleidoscopic mandala from video."""
    def __init__(self):
        super().__init__()

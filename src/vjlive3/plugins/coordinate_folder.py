"""
P7-VE62: Coordinate folder — fold/mirror UV coordinates.
...
METADATA = {
    "name": "Coordinate Folder",
    "description": "Coordinate folder — fold/mirror UV coordinates.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['coordinate', 'fold', 'uv', 'transform', 'kaleidoscope'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'fold_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'fold_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'offset_x', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'offset_y', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float fold_x;  // X fold count
uniform float fold_y;  // Y fold count
uniform float offset_x;  // X offset
uniform float offset_y;  // Y offset
...
_PARAM_NAMES = ['fold_x', 'fold_y', 'offset_x', 'offset_y', 'rotation', 'mix']
_PARAM_DEFAULTS = {'fold_x': 5.0, 'fold_y': 5.0, 'offset_x': 0.0, 'offset_y': 0.0, 'rotation': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class CoordinateFolderPlugin(EffectPlugin):
    """Coordinate folder — fold/mirror UV coordinates."""
    def __init__(self):
        super().__init__()

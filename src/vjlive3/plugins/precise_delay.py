"""
P7-VE64: Precise delay — exact frame delay buffer.
...
METADATA = {
    "name": "Precise Delay",
    "description": "Precise delay — exact frame delay buffer.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['delay', 'temporal', 'buffer', 'latency', 'timing'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'delay_frames', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'freeze', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'jitter', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float delay_frames;  // Frame delay
uniform float blend;  // Blend ratio
uniform float freeze;  // Freeze frame
uniform float jitter;  // Random jitter
uniform float color_shift;  // Hue drift
...
void main() {
    ...
    float d = 0.0; float fb = (blend/10.0); float fr = 8.0+0.0*10.0;
...
_PARAM_NAMES = ['delay_frames', 'blend', 'freeze', 'jitter', 'color_shift', 'mix']
_PARAM_DEFAULTS = {'delay_frames': 0.0, 'blend': 5.0, 'freeze': 0.0, 'jitter': 0.0, 'color_shift': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class PreciseDelayPlugin(EffectPlugin):
    """Precise delay — exact frame delay buffer."""
    def __init__(self):
        super().__init__()

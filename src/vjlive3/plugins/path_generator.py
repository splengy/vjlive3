"""
P6-GE14: Path generator — draws SVG-style Bezier paths through the frame.
...
METADATA = {
    "name": "Path Generator",
    "description": "Path generator — draws SVG-style Bezier paths through the frame.",
    "version": "1.0.0",
    "plugin_type": "generator",
    "category": "generator",
    "tags": ['path', 'bezier', 'line', 'generative', 'animation'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'path_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'path_width', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'path_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'curvature', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_cycle', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'glow', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'fade', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'noise_warp', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'fork_chance', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_follow', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float path_count;  // Path count
uniform float path_width;  // Line width
uniform float path_speed;  // Animation speed
uniform float curvature;  // Curve bend
uniform float color_hue;  // Base hue
uniform float color_cycle;  // Hue cycling
uniform float glow;  // Path glow
uniform float fade;  // Fade trail
uniform float feedback;  // Feedback
uniform float noise_warp;  // Noise warp
uniform float fork_chance;  // Branching
uniform float depth_follow;  // Depth follow
...
void main() {
    ...
    float freq_v   = 8.0 + (path_speed / 10.0) * 10.0;
    float chroma   = 0.0;
    float vign_v   = 0.0;
...
_PARAM_NAMES = ['path_count', 'path_width', 'path_speed', 'curvature', 'color_hue', 'color_cycle', 'glow', 'fade', 'feedback', 'noise_warp', 'fork_chance', 'depth_follow']
_PARAM_DEFAULTS = {'path_count': 5.0, 'path_width': 3.0, 'path_speed': 5.0, 'curvature': 5.0, 'color_hue': 5.0, 'color_cycle': 3.0, 'glow': 3.0, 'fade': 5.0, 'feedback': 0.0, 'noise_warp': 3.0, 'fork_chance': 3.0, 'depth_follow': 0.0}
...
class PathGeneratorPlugin(EffectPlugin):

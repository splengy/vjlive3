"""
P7-VE15: Bloom — HDR-style glow bloom on bright areas.
...
METADATA = {
    "name": "Bloom Effect",
    "description": "Bloom — HDR-style glow bloom on bright areas.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['bloom', 'glow', 'hdr', 'bright', 'luminance'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'knee', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float strength;  // Bloom intensity
uniform float saturation;  // Bloom saturation
uniform float knee;  // Soft knee
...
_PARAM_NAMES = ['threshold', 'radius', 'strength', 'saturation', 'knee', 'mix']
_PARAM_DEFAULTS = {'threshold': 5.0, 'radius': 5.0, 'strength': 5.0, 'saturation': 5.0, 'knee': 3.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BloomPlugin(EffectPlugin):
    """Bloom — HDR-style glow bloom on bright areas."""
    def __init__(self):
        super().__init__()

"""
P7-VE17: Mixer — A/B video source mixer with crossfade.
...
METADATA = {
    "name": "Mixer Effect",
    "description": "Mixer — A/B video source mixer with crossfade.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['mixer', 'crossfade', 'blend', 'a-b', 'composite'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'crossfade', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gain_a', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gain_b', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float crossfade;  // A/B crossfade
uniform float gain_a;  // Input A gain
uniform float gain_b;  // Input B gain
...
void main() {
    ...
    c.r *= 1.+0.0; c.g *= 1.+gain_a/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['crossfade', 'gain_a', 'gain_b', 'gamma', 'blend_mode', 'mix']
_PARAM_DEFAULTS = {'crossfade': 5.0, 'gain_a': 5.0, 'gain_b': 5.0, 'gamma': 5.0, 'blend_mode': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class MixerPlugin(EffectPlugin):
    """Mixer — A/B video source mixer with crossfade."""
    def __init__(self):
        super().__init__()

"""
P7-VE19: Chroma key — green/blue screen removal.
...
METADATA = {
    ...
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['chroma-key', 'green-screen', 'compositing', 'mask', 'keying'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'key_hue', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'tolerance', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_feather', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'spill_supp', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'background_blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float key_hue;  // Key hue (0=green,5=blue)
uniform float tolerance;  // Hue tolerance
uniform float edge_feather;  // Edge feather
uniform float spill_supp;  // Spill suppression
uniform float background_blend;  // BG blend
...
void main() {
    ...
    c.r *= 1.+key_hue/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['key_hue', 'tolerance', 'edge_feather', 'spill_supp', 'background_blend', 'mix']
_PARAM_DEFAULTS = {'key_hue': 3.0, 'tolerance': 3.0, 'edge_feather': 3.0, 'spill_supp': 3.0, 'background_blend': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ChromaKeyPlugin(EffectPlugin):

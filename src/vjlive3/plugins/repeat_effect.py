"""
P7-VE40: Repeat — tile and repeat input video.
...
METADATA = {
    "name": "Repeat Effect",
    "description": "Repeat — tile and repeat input video.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['repeat', 'tile', 'grid', 'layout', 'transform'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'count_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'count_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'offset_x', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'offset_y', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mirror', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float count_x;  // Horizontal tiles
uniform float count_y;  // Vertical tiles
uniform float offset_x;  // X offset
uniform float offset_y;  // Y offset
uniform float mirror;  // Mirror alternates
...
_PARAM_NAMES = ['count_x', 'count_y', 'offset_x', 'offset_y', 'mirror', 'mix']
_PARAM_DEFAULTS = {'count_x': 5.0, 'count_y': 5.0, 'offset_x': 0.0, 'offset_y': 0.0, 'mirror': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class RepeatPlugin(EffectPlugin):
    """Repeat — tile and repeat input video."""
    def __init__(self):
        super().__init__()

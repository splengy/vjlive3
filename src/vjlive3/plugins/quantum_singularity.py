"""
P6-QC14: Quantum consciousness singularity — collapse to a point of infinite density.
...
METADATA = {
    ...
    "description": "Quantum consciousness singularity — collapse to a point of infinite density.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['quantum', 'singularity', 'collapse', 'vortex', 'consciousness'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'singularity_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'event_horizon', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'time_dilation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'hawking_glow', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'spaghetti', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'quantum_foam', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'consciousness_echo', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float singularity_strength;  // Pull strength
uniform float rotation_speed;  // Rotation
uniform float event_horizon;  // Event horizon size
uniform float time_dilation;  // Time dilation
uniform float hawking_glow;  // Hawking radiation
uniform float spaghetti;  // Tidal distortion
uniform float quantum_foam;  // Quantum foam
uniform float consciousness_echo;  // Echo
...
void main() {
    ...
    float disp     = (singularity_strength / 10.0);
    float feed     = 0.0;
    ...
    color.r *= 1.0 + hawking_glow/10.0*0.3;
    color.g *= 1.0 + hawking_glow/10.0*0.2;
    color.b *= 1.0 + quantum_foam/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['singularity_strength', 'rotation_speed', 'event_horizon', 'time_dilation', 'hawking_glow', 'spaghetti', 'quantum_foam', 'consciousness_echo']
_PARAM_DEFAULTS = {'singularity_strength': 5.0, 'rotation_speed': 5.0, 'event_horizon': 5.0, 'time_dilation': 5.0, 'hawking_glow': 3.0, 'spaghetti': 5.0, 'quantum_foam': 3.0, 'consciousness_echo': 5.0}
...
class QuantumSingularityPlugin(EffectPlugin):

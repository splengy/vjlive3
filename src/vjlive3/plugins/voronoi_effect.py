"""
P6-GE09: Voronoi generator — cellular patterns with dynamic seed movement.
...
METADATA = {
    "name": "Voronoi Effect",
    "description": "Voronoi generator — cellular patterns with dynamic seed movement.",
    "version": "1.0.0",
    "plugin_type": "generator",
    "category": "generator",
    "tags": ['voronoi', 'cellular', 'geometric', 'generative', 'pattern'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'cell_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'border_width', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_mode', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'distance_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'fade_edges', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'palette_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'palette_range', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'z_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'noise_warp', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'pulse', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback_mix', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float cell_count;  // Cell count
uniform float speed;  // Animation speed
uniform float border_width;  // Border thickness
uniform float color_mode;  // Color mode
uniform float distance_mode;  // Distance metric
uniform float fade_edges;  // Edge fade
uniform float palette_hue;  // Base hue
uniform float palette_range;  // Hue spread
uniform float z_depth;  // Z-depth 3D
uniform float noise_warp;  // Noise warp
uniform float pulse;  // Cell pulse
uniform float feedback_mix;  // Feedback
...
void main() {
    ...
    float disp     = 0.0;
    float feed     = 0.0;
    float freq_v   = 8.0 + (speed / 10.0) * 10.0;
    float chroma   = 0.0;
    float vign_v   = 0.0;
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + feedback_mix/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['cell_count', 'speed', 'border_width', 'color_mode', 'distance_mode', 'fade_edges', 'palette_hue', 'palette_range', 'z_depth', 'noise_warp', 'pulse', 'feedback_mix']
_PARAM_DEFAULTS = {'cell_count': 5.0, 'speed': 5.0, 'border_width': 3.0, 'color_mode': 5.0, 'distance_mode': 0.0, 'fade_edges': 5.0, 'palette_hue': 5.0, 'palette_range': 5.0, 'z_depth': 5.0, 'noise_warp': 3.0, 'pulse': 3.0, 'feedback_mix': 0.0}
...
class VoronoiPlugin(EffectPlugin):

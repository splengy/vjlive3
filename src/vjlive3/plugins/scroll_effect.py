"""
P7-VE41: Scroll — continuous UV scrolling.
...
METADATA = {
    "name": "Scroll Effect",
    "description": "Scroll — continuous UV scrolling.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['scroll', 'pan', 'uv', 'motion', 'camera'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'speed_x', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed_y', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'wrap', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'blur_edges', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'zoom', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float speed_x;  // Horizontal speed
uniform float speed_y;  // Vertical speed
uniform float wrap;  // Wrap edges
...
void main() {
    ...
    float d = (zoom/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+blur_edges/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+zoom/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['speed_x', 'speed_y', 'wrap', 'blur_edges', 'zoom', 'mix']
_PARAM_DEFAULTS = {'speed_x': 0.0, 'speed_y': 0.0, 'wrap': 8.0, 'blur_edges': 0.0, 'zoom': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ScrollPlugin(EffectPlugin):
    """Scroll — continuous UV scrolling."""
    def __init__(self):
        super().__init__()

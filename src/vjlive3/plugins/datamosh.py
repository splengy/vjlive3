"""
P5-DM11: Core datamosh — frame differencing, temporal displacement, codec corruption.
Ported from VJlive-2/plugins/vdatamosh/datamosh.py.
"""
...
METADATA = {
    "name": "Datamosh",
    "description": "Core datamosh — frame differencing, temporal displacement, codec corruption.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['datamosh', 'glitch', 'temporal', 'compression', 'core'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
...
uniform float time;
uniform float u_mix;
uniform float intensity;  // Overall datamosh strength
uniform float scale;  // Block scale
uniform float speed;  // Temporal speed
uniform float threshold;  // Motion threshold
uniform float foreground_mix;  // FG mix
uniform float background_mix;  // BG mix
uniform float glitch_amount;  // Glitch intensity

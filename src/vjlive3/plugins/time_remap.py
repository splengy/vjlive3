"""
P7-VE67: Time remap — non-linear time remapping.
...
METADATA = {
    "name": "Time Remap",
    "description": "Time remap — non-linear time remapping.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['time-remap', 'speed', 'slow-motion', 'reverse', 'temporal'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'speed_mult', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'reverse', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'freeze', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'stutter', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float speed_mult;  // Playback speed
uniform float reverse;  // Reverse
uniform float freeze;  // Freeze point
uniform float stutter;  // Stutter amount
uniform float blend;  // Inter-frame blend
...
_PARAM_NAMES = ['speed_mult', 'reverse', 'freeze', 'stutter', 'blend', 'mix']
_PARAM_DEFAULTS = {'speed_mult': 5.0, 'reverse': 0.0, 'freeze': 0.0, 'stutter': 0.0, 'blend': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class TimeRemapPlugin(EffectPlugin):
    """Time remap — non-linear time remapping."""
    def __init__(self):
        super().__init__()

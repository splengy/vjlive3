"""
P7-VE39: Pixelate — blocky mosaic pixelation.
...
METADATA = {
    ...
    "description": "Pixelate — blocky mosaic pixelation.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['pixelate', 'mosaic', 'blocky', '8bit', 'stylize'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'block_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'shape', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_blend', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_detect', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'animation_speed', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float shape;  // Block shape
uniform float color_blend;  // Color average
uniform float edge_detect;  // Edge preserve
uniform float animation_speed;  // Block jitter
...
void main() {
    ...
    float d = 0.0; float fb = 0.0; float fr = 8.0+(animation_speed/10.0)*10.0;
    ...
    c.r *= 1.+color_blend/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['block_size', 'shape', 'color_blend', 'edge_detect', 'animation_speed', 'mix']
_PARAM_DEFAULTS = {'block_size': 3.0, 'shape': 0.0, 'color_blend': 5.0, 'edge_detect': 0.0, 'animation_speed': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class PixelatePlugin(EffectPlugin):
    """Pixelate — blocky mosaic pixelation."""
    def __init__(self):
        super().__init__()

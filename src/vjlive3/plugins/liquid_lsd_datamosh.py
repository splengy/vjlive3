"""
P5-DM23: Liquid LSD — oil-layer psychedelic flow with color cycling.
Ported from VJlive-2/plugins/vdatamosh/liquid_lsd_datamosh.py.
"""
...
METADATA = {
    "name": "Liquid Lsd Datamosh",
    "description": "Liquid LSD — oil-layer psychedelic flow with color cycling.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['lsd', 'psychedelic', 'liquid', 'datamosh', 'trippy'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'viscosity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_cycle', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'oil_layers', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'flow_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'melt_amount', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'tracer_length', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'distortion', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_shimmer', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'psyche_boost', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'feedback_zoom', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'surface_tension', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'wave_frequency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float viscosity;  // Fluid thickness
uniform float color_cycle;  // Hue cycling speed
uniform float oil_layers;  // Layer count
uniform float flow_speed;  // Flow rate
uniform float melt_amount;  // Melt amount
uniform float tracer_length;  // Tracer trail
uniform float distortion;  // UV distortion
uniform float edge_shimmer;  // Edge shimmer
uniform float psyche_boost;  // Intensity boost
uniform float feedback_zoom;  // Feedback zoom
uniform float surface_tension;  // Surface tension
uniform float wave_frequency;  // Wave freq
...
void main() {
    ...
    float displacement = (viscosity / 10.0);
    float feedback_v   = (wave_frequency / 10.0);
    float freq         = 8.0 + (flow_speed / 10.0) * 10.0;
    float chroma       = 0.0;
    float vign_v       = 0.0;
...
_PARAM_NAMES = ['viscosity', 'color_cycle', 'oil_layers', 'flow_speed', 'melt_amount', 'tracer_length', 'distortion', 'edge_shimmer', 'psyche_boost', 'feedback_zoom', 'surface_tension', 'wave_frequency']
_PARAM_DEFAULTS = {'viscosity': 5.0, 'color_cycle': 5.0, 'oil_layers': 5.0, 'flow_speed': 5.0, 'melt_amount': 5.0, 'tracer_length': 5.0, 'distortion': 5.0, 'edge_shimmer': 3.0, 'psyche_boost': 5.0, 'feedback_zoom': 3.0, 'surface_tension': 5.0, 'wave_frequency': 5.0}
...
class LiquidLSDDatamoshPlugin(EffectPlugin):

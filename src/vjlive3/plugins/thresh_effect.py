"""
P7-VE26: Threshold — converts to binary B/W.
...
METADATA = {
    "name": "Thresh Effect",
    "description": "Threshold — converts to binary B/W.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['threshold', 'binary', 'mask', 'stylize', 'bold'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'softness', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'invert', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_high', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_low', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float threshold;  // Luma threshold
uniform float softness;  // Edge softness
uniform float invert;  // Invert
uniform float color_high;  // High color
uniform float color_low;  // Low color
...
void main() {
    ...
    c.r *= 1.+color_high/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['threshold', 'softness', 'invert', 'color_high', 'color_low', 'mix']
_PARAM_DEFAULTS = {'threshold': 5.0, 'softness': 3.0, 'invert': 0.0, 'color_high': 8.0, 'color_low': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ThreshPlugin(EffectPlugin):
    """Threshold — converts to binary B/W."""
    def __init__(self):
        super().__init__()

"""
P7-VE70: HDR tone map — Reinhard/ACES tone mapping.
...
METADATA = {
    "name": "Hdr Tonemap",
    "description": "HDR tone map — Reinhard/ACES tone mapping.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['hdr', 'tonemapping', 'reinhard', 'aces', 'exposure'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'exposure', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'tone_mode', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'white_point', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float tone_mode;  // 0=linear,3=reinhard,5=aces
uniform float white_point;  // White point
...
_PARAM_NAMES = ['exposure', 'tone_mode', 'white_point', 'gamma', 'saturation', 'mix']
_PARAM_DEFAULTS = {'exposure': 5.0, 'tone_mode': 5.0, 'white_point': 8.0, 'gamma': 5.0, 'saturation': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class HDRToneMapPlugin(EffectPlugin):
    """HDR tone map — Reinhard/ACES tone mapping."""
    def __init__(self):
        super().__init__()

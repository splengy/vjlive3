"""
P7-VE42: Mirror — flip/mirror video input.
...
METADATA = {
    "name": "Mirror Effect",
    "description": "Mirror — flip/mirror video input.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['mirror', 'flip', 'symmetry', 'horizontal', 'vertical'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'mirror_x', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mirror_y', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'quad_mirror', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float mirror_x;  // Horizontal mirror
uniform float mirror_y;  // Vertical mirror
uniform float center_x;  // Center X
uniform float center_y;  // Center Y
uniform float quad_mirror;  // 4-way mirror
...
void main() {
    ...
    c.r *= 1.+mirror_x/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['mirror_x', 'mirror_y', 'center_x', 'center_y', 'quad_mirror', 'mix']
_PARAM_DEFAULTS = {'mirror_x': 0.0, 'mirror_y': 0.0, 'center_x': 5.0, 'center_y': 5.0, 'quad_mirror': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class MirrorPlugin(EffectPlugin):
    """Mirror — flip/mirror video input."""
    def __init__(self):
        super().__init__()

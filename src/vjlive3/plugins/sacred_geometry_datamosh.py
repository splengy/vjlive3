"""
P5-DM29: Sacred geometry — flower of life, merkaba spin, mandala folding.
Ported from VJlive-2/plugins/vdatamosh/sacred_geometry_datamosh.py.
"""
...
METADATA = {
    "name": "Sacred Geometry Datamosh",
    "description": "Sacred geometry — flower of life, merkaba spin, mandala folding.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['sacred', 'geometry', 'mandala', 'datamosh', 'spiritual'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'geo_scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'flower_mix', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'merkaba_spin', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mandala_folds', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'golden_spiral', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'line_width', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'glow_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_bloom', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'divine_light', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pattern_switch', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_geo', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'sym_center_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float geo_scale;  // Geometry scale
uniform float flower_mix;  // Flower of life
uniform float merkaba_spin;  // Merkaba spin
uniform float mandala_folds;  // Mandala folds
uniform float golden_spiral;  // Golden ratio spiral
uniform float line_width;  // Line width
uniform float glow_strength;  // Glow
uniform float depth_bloom;  // Depth bloom
uniform float divine_light;  // Light intensity
uniform float pattern_switch;  // Pattern select
uniform float chroma_geo;  // Chromatic geo
uniform float sym_center_x;  // Center X
...
void main() {
    ...
    float displacement = (geo_scale / 10.0);
    float feedback_v   = (sym_center_x / 10.0);
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + glow_strength/10.0*0.2;
    color.b *= 1.0 + depth_bloom/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['geo_scale', 'flower_mix', 'merkaba_spin', 'mandala_folds', 'golden_spiral', 'line_width', 'glow_strength', 'depth_bloom', 'divine_light', 'pattern_switch', 'chroma_geo', 'sym_center_x']
_PARAM_DEFAULTS = {'geo_scale': 5.0, 'flower_mix': 5.0, 'merkaba_spin': 3.0, 'mandala_folds': 5.0, 'golden_spiral': 5.0, 'line_width': 3.0, 'glow_strength': 5.0, 'depth_bloom': 3.0, 'divine_light': 5.0, 'pattern_switch': 5.0, 'chroma_geo': 3.0, 'sym_center_x': 5.0}
...
class SacredGeometryDatamoshPlugin(EffectPlugin):

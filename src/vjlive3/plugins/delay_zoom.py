"""
P7-VE75: Delay zoom — temporal zoom echo cascade.
...
METADATA = {
    "name": "Delay Zoom",
    "description": "Delay zoom — temporal zoom echo cascade.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['delay', 'zoom', 'temporal', 'echo', 'cascade'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'zoom_per_echo', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'echo_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_drift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float zoom_per_echo;  // Zoom factor
uniform float echo_count;  // Echo count
uniform float decay;  // Opacity decay
uniform float rotation;  // Rotation per echo
uniform float hue_drift;  // Hue per echo
...
void main() {
    ...
    float d = 0.0; float fb = (decay/10.0); float fr = 8.0+(echo_count/10.0)*10.0;
    ...
    c.r *= 1.+zoom_per_echo/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+zoom_per_echo/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['zoom_per_echo', 'echo_count', 'decay', 'rotation', 'hue_drift', 'mix']
_PARAM_DEFAULTS = {'zoom_per_echo': 5.0, 'echo_count': 5.0, 'decay': 5.0, 'rotation': 0.0, 'hue_drift': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class DelayZoomPlugin(EffectPlugin):
    """Delay zoom — temporal zoom echo cascade."""
    def __init__(self):
        super().__init__()

"""
P7-VE80: Visualizer — general-purpose audio visualizer.
...
METADATA = {
    "name": "Visualizer Effect",
    "description": "Visualizer — general-purpose audio visualizer.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['visualizer', 'audio', 'spectrum', 'bars', 'music'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'bar_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bar_height', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_mode', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_response', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float bar_count;  // Bar count
uniform float bar_height;  // Height scale
uniform float color_hue;  // Base hue
uniform float color_mode;  // Color from frequency
...
void main() {
    ...
    c.r *= 1.+bar_count/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['bar_count', 'bar_height', 'color_hue', 'color_mode', 'bass_response', 'mix']
_PARAM_DEFAULTS = {'bar_count': 5.0, 'bar_height': 5.0, 'color_hue': 5.0, 'color_mode': 5.0, 'bass_response': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class VisualizerPlugin(EffectPlugin):
    """Visualizer — general-purpose audio visualizer."""
    def __init__(self):
        super().__init__()

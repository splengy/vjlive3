"""
P7-VE60: Video out — pass-through output with gain.
...
METADATA = {
    "name": "Video Out",
    "description": "Video out — pass-through output with gain.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['output', 'passthrough', 'final', 'gain', 'master'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'gain', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'black_level', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'white_level', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float gain;  // Output gain
uniform float gamma;  // Gamma
uniform float black_level;  // Black clip
uniform float white_level;  // White clip
...
_PARAM_NAMES = ['gain', 'gamma', 'black_level', 'white_level', 'saturation', 'mix']
_PARAM_DEFAULTS = {'gain': 5.0, 'gamma': 5.0, 'black_level': 0.0, 'white_level': 10.0, 'saturation': 5.0, 'mix': 10.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class VideoOutPlugin(EffectPlugin):
    """Video out — pass-through output with gain."""
    def __init__(self):
        super().__init__()

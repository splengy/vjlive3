"""
P7-VE03: Vertical wave — sine UV warp along Y axis.
...
METADATA = {
    "name": "Vertical Wave",
    "description": "Vertical wave — sine UV warp along Y axis.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['wave', 'vertical', 'distortion', 'uv', 'warp'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amplitude', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'frequency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'phase', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'sharpness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float amplitude;  // Wave width
uniform float frequency;  // Wave count
uniform float speed;  // Scroll speed
uniform float phase;  // Phase
uniform float sharpness;  // Wave sharpness
...
_PARAM_NAMES = ['amplitude', 'frequency', 'speed', 'phase', 'sharpness', 'mix']
_PARAM_DEFAULTS = {'amplitude': 5.0, 'frequency': 5.0, 'speed': 5.0, 'phase': 0.0, 'sharpness': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class VerticalWavePlugin(EffectPlugin):
    """Vertical wave — sine UV warp along Y axis."""
    def __init__(self):
        super().__init__()

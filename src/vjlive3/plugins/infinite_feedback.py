"""
P7-VE14: Infinite feedback — recursive zoom-in feedback.
...
METADATA = {
    ...
    "description": "Infinite feedback — recursive zoom-in feedback.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['feedback', 'infinite', 'zoom', 'recursive', 'trippy'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'zoom', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'decay', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_drift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'chromatic', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float zoom;  // Zoom per frame
uniform float decay;  // Decay rate
uniform float rotation;  // Rotation per frame
uniform float hue_drift;  // Hue drift
uniform float chromatic;  // Chromatic drift
...
void main() {
    ...
    float d = (zoom/10.0); float fb = (decay/10.0); float fr = 8.0+0.0*10.0;
    float ch = (chromatic/10.0); float vg = 0.0;
    ...
    c.r *= 1.+hue_drift/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+zoom/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['zoom', 'decay', 'rotation', 'hue_drift', 'chromatic', 'mix']
_PARAM_DEFAULTS = {'zoom': 5.0, 'decay': 3.0, 'rotation': 0.0, 'hue_drift': 0.0, 'chromatic': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class InfiniteFeedbackPlugin(EffectPlugin):
    """Infinite feedback — recursive zoom-in feedback."""
    def __init__(self):
        super().__init__()

"""
P7-VE58: Resize — letterbox/crop/stretch resize modes.
...
METADATA = {
    "name": "Resize Effect",
    "description": "Resize — letterbox/crop/stretch resize modes.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['resize', 'crop', 'letterbox', 'transform', 'aspect'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'target_width', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'target_height', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'offset_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'blur_edges', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float target_width;  // Target width %
uniform float target_height;  // Target height %
uniform float mode;  // 0=stretch,3=fit,5=crop,10=fill
uniform float offset_x;  // Offset X
uniform float blur_edges;  // Edge blur
...
void main() {
    ...
    c.r *= 1.+blur_edges/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['target_width', 'target_height', 'mode', 'offset_x', 'blur_edges', 'mix']
_PARAM_DEFAULTS = {'target_width': 5.0, 'target_height': 5.0, 'mode': 0.0, 'offset_x': 5.0, 'blur_edges': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ResizePlugin(EffectPlugin):
    """Resize — letterbox/crop/stretch resize modes."""
    def __init__(self):
        super().__init__()

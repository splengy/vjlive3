"""
P7-VE49: Milkdrop — classic Winamp-style audio visualizer.
...
METADATA = {
    "name": "Milkdrop",
    "description": "Milkdrop — classic Winamp-style audio visualizer.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['milkdrop', 'visualizer', 'audio', 'preset', 'winamp'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'preset_mix', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'wave_size', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'warp_amount', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_response', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float preset_mix;  // Preset blend
uniform float wave_size;  // Wave size
uniform float warp_amount;  // UV warp
uniform float color_speed;  // Color speed
...
void main() {
    ...
    float d = (warp_amount/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+color_speed/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['preset_mix', 'wave_size', 'warp_amount', 'color_speed', 'bass_response', 'mix']
_PARAM_DEFAULTS = {'preset_mix': 5.0, 'wave_size': 5.0, 'warp_amount': 5.0, 'color_speed': 5.0, 'bass_response': 5.0, 'mix': 8.0}

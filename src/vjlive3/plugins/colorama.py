"""
P7-VE30: Colorama — maps luma to a hue gradient.
...
METADATA = {
    "name": "Colorama",
    "description": "Colorama — maps luma to a hue gradient.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['colorama', 'luma-to-color', 'gradient', 'psychedelic', 'abstract'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'hue_start', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_end', 'type': 'float', 'default': 10.0, 'min': 0.0, 'max': 10.0}, {'name': 'saturation', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}, {'name': 'cycle_speed', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'gamma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float hue_end;  // End hue
uniform float saturation;  // Saturation
uniform float cycle_speed;  // Cycle speed
uniform float gamma;  // Gamma pre-map
...
void main() {
    ...
    c.r *= 1.+hue_start/10.0*0.2; c.g *= 1.+saturation/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['hue_start', 'hue_end', 'saturation', 'cycle_speed', 'gamma', 'mix']
_PARAM_DEFAULTS = {'hue_start': 0.0, 'hue_end': 10.0, 'saturation': 8.0, 'cycle_speed': 0.0, 'gamma': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ColoramaPlugin(EffectPlugin):
    """Colorama — maps luma to a hue gradient."""
    def __init__(self):
        super().__init__()

"""
P7-VE04: Ripple — radial sinusoidal ripple from center.
...
METADATA = {
    "name": "Ripple Effect",
    "description": "Ripple — radial sinusoidal ripple from center.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['ripple', 'radial', 'water', 'distortion', 'wave'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'amplitude', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'frequency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_x', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'center_y', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float frequency;  // Ripple freq
uniform float speed;  // Propagation
uniform float center_x;  // Origin X
uniform float center_y;  // Origin Y
...
void main() {
    ...
    float d = (amplitude/10.0); float fb = 0.0; float fr = 8.0+(frequency/10.0)*10.0;
    ...
    c.r *= 1.+center_x/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['amplitude', 'frequency', 'speed', 'center_x', 'center_y', 'mix']
_PARAM_DEFAULTS = {'amplitude': 5.0, 'frequency': 5.0, 'speed': 5.0, 'center_x': 5.0, 'center_y': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class RipplePlugin(EffectPlugin):
    """Ripple — radial sinusoidal ripple from center."""
    def __init__(self):
        super().__init__()

"""
P7-VE07: Bass cannon 2 — upgraded shockwave bass cannon.
...
METADATA = {
    "name": "Bass Cannon 2",
    "description": "Bass cannon 2 — upgraded shockwave bass cannon.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['bass', 'shockwave', 'audio', 'impact', 'cannon'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'cannon_power', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'shockwave_radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'recoil', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'flash', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_burst', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float cannon_power;  // Shockwave power
uniform float shockwave_radius;  // Blast radius
uniform float recoil;  // Screen recoil
uniform float flash;  // Muzzle flash
uniform float chroma_burst;  // Chromatic burst
...
void main() {
    ...
    c.r *= 1.+0.0; c.g *= 1.+0.0; c.b *= 1.+cannon_power/10.0*0.2;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['cannon_power', 'shockwave_radius', 'recoil', 'flash', 'chroma_burst', 'mix']
_PARAM_DEFAULTS = {'cannon_power': 5.0, 'shockwave_radius': 5.0, 'recoil': 3.0, 'flash': 2.0, 'chroma_burst': 3.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BassCanon2Plugin(EffectPlugin):
    """Bass cannon 2 — upgraded shockwave bass cannon."""
    def __init__(self):
        super().__init__()

"""
P7-VE45: Hyperspace tunnel — warp-speed star field tunnel.
...
METADATA = {
    ...
    "description": "Hyperspace tunnel — warp-speed star field tunnel.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['hyperspace', 'tunnel', 'warp', 'stars', 'space'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'twist', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'star_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float speed;  // Warp speed
uniform float radius;  // Tunnel radius
uniform float twist;  // Tunnel twist
uniform float star_density;  // Star density
uniform float color_shift;  // Hue shift
...
void main() {
    ...
    float d = 0.0; float fb = 0.0; float fr = 8.0+(speed/10.0)*10.0;
    ...
    c.r *= 1.+star_density/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['speed', 'radius', 'twist', 'star_density', 'color_shift', 'mix']
_PARAM_DEFAULTS = {'speed': 5.0, 'radius': 5.0, 'twist': 3.0, 'star_density': 5.0, 'color_shift': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class HyperspaceTunnelPlugin(EffectPlugin):
    """Hyperspace tunnel — warp-speed star field tunnel."""
    def __init__(self):
        super().__init__()

"""
P5-DM25: Neural splice — synapse firings, axon pathways, hallucination patterns.
Ported from VJlive-2/plugins/vdatamosh/neural_splice_datamosh.py.
"""
...
METADATA = {
    "name": "Neural Splice Datamosh",
    "description": "Neural splice — synapse firings, axon pathways, hallucination patterns.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['neural', 'synapse', 'brain', 'datamosh', 'organic'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'synapse_str', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'pathway_width', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'firing_rate', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'inhibition', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'dendrite_spread', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'axon_length', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'myelin_decay', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'cross_talk', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'halluc_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_sens', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'phase_lock', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'resonance', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float synapse_str;  // Synapse strength
uniform float pathway_width;  // Pathway width
uniform float firing_rate;  // Firing rate
uniform float inhibition;  // Inhibition
uniform float dendrite_spread;  // Dendrite spread
uniform float axon_length;  // Axon length
uniform float myelin_decay;  // Myelin decay
uniform float cross_talk;  // Crosstalk
uniform float halluc_depth;  // Hallucination depth
uniform float edge_sens;  // Edge sensitivity
uniform float phase_lock;  // Phase lock
uniform float resonance;  // Resonance
...
void main() {
    ...
    float displacement = (synapse_str / 10.0);
    float feedback_v   = (resonance / 10.0);
    float freq         = 8.0 + (firing_rate / 10.0) * 10.0;
    float chroma       = (cross_talk / 10.0);
    float vign_v       = 0.0;
...
_PARAM_NAMES = ['synapse_str', 'pathway_width', 'firing_rate', 'inhibition', 'dendrite_spread', 'axon_length', 'myelin_decay', 'cross_talk', 'halluc_depth', 'edge_sens', 'phase_lock', 'resonance']
_PARAM_DEFAULTS = {'synapse_str': 5.0, 'pathway_width': 3.0, 'firing_rate': 5.0, 'inhibition': 3.0, 'dendrite_spread': 5.0, 'axon_length': 5.0, 'myelin_decay': 3.0, 'cross_talk': 3.0, 'halluc_depth': 5.0, 'edge_sens': 5.0, 'phase_lock': 5.0, 'resonance': 5.0}
...
class NeuralSpliceDatamoshPlugin(EffectPlugin):

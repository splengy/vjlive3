"""
P7-VE68: Gaussian blur — separable Gaussian blur.
...
METADATA = {
    "name": "Gaussian Blur",
    "description": "Gaussian blur — separable Gaussian blur.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['blur', 'gaussian', 'smooth', 'soften', 'denoise'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'radius', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'sigma', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'quality', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'edge_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_bleed', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float quality;  // Pass count
uniform float edge_mode;  // Edge handling
uniform float color_bleed;  // Chromatic blur
...
void main() {
    ...
    float ch = (color_bleed/10.0); float vg = 0.0;
    ...
    c.r *= 1.+color_bleed/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['radius', 'sigma', 'quality', 'edge_mode', 'color_bleed', 'mix']
_PARAM_DEFAULTS = {'radius': 5.0, 'sigma': 5.0, 'quality': 5.0, 'edge_mode': 0.0, 'color_bleed': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class GaussianBlurPlugin(EffectPlugin):
    """Gaussian blur — separable Gaussian blur."""
    def __init__(self):
        super().__init__()

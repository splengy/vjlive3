"""
P5-DM22: Fracture rave — split-screen fractures, laser beams, bass pulse.
Ported from VJlive-2/plugins/vdatamosh/fracture_rave_datamosh.py.
"""
...
METADATA = {
    "name": "Fracture Rave Datamosh",
    "description": "Fracture rave — split-screen fractures, laser beams, bass pulse.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['rave', 'laser', 'fracture', 'datamosh', 'neon'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'fracture_sens', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'fracture_width', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'fracture_glow', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'chromatic_split', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'laser_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'laser_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'laser_hue', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_pulse', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'body_glow', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'euphoria', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'trails', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mosh_intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float fracture_sens;  // Fracture sensitivity
uniform float fracture_width;  // Fracture width
uniform float fracture_glow;  // Edge glow
uniform float chromatic_split;  // Chromatic separation
uniform float laser_count;  // Laser count
uniform float laser_speed;  // Laser speed
uniform float laser_hue;  // Laser color
uniform float bass_pulse;  // Bass pulse
uniform float body_glow;  // Body glow
uniform float euphoria;  // Joy intensity
uniform float trails;  // Trail length
uniform float mosh_intensity;  // Mosh amount
...
void main() {
    ...
    float feedback_v   = (mosh_intensity / 10.0);
    ...
    color.r *= 1.0 + laser_hue/10.0*0.3;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + 0.0;
...
_PARAM_NAMES = ['fracture_sens', 'fracture_width', 'fracture_glow', 'chromatic_split', 'laser_count', 'laser_speed', 'laser_hue', 'bass_pulse', 'body_glow', 'euphoria', 'trails', 'mosh_intensity']
_PARAM_DEFAULTS = {'fracture_sens': 5.0, 'fracture_width': 3.0, 'fracture_glow': 5.0, 'chromatic_split': 3.0, 'laser_count': 5.0, 'laser_speed': 5.0, 'laser_hue': 5.0, 'bass_pulse': 5.0, 'body_glow': 3.0, 'euphoria': 5.0, 'trails': 5.0, 'mosh_intensity': 5.0}
...
class FractureRaveDatamoshPlugin(EffectPlugin):

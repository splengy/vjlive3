"""
P5-DM18: Shatter — depth map triggers shattering glass patterns.
Ported from VJlive-2/plugins/vdatamosh/shatter_datamosh.py.
"""
...
METADATA = {
    "name": "Shatter Datamosh",
    "description": "Shatter — depth map triggers shattering glass patterns.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['shatter', 'glass', 'depth', 'datamosh', 'destruction'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'intensity', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'scale', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'threshold', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'foreground_mix', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'background_mix', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'glitch_amount', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'block_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float intensity;  // Shatter strength
uniform float scale;  // Fragment size
uniform float speed;  // Explosion speed
uniform float threshold;  // Depth trigger
uniform float foreground_mix;  // FG mix
uniform float background_mix;  // BG mix
uniform float glitch_amount;  // Glitch
...
void main() {
    ...
    float feedback_v   = (block_size / 10.0);
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + glitch_amount/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['intensity', 'scale', 'speed', 'threshold', 'foreground_mix', 'background_mix', 'glitch_amount', 'block_size']
_PARAM_DEFAULTS = {'intensity': 5.0, 'scale': 5.0, 'speed': 5.0, 'threshold': 5.0, 'foreground_mix': 5.0, 'background_mix': 5.0, 'glitch_amount': 3.0, 'block_size': 3.0}
...
class ShatterDatamoshPlugin(EffectPlugin):

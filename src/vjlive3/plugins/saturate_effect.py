"""
P7-VE22: Saturate — HSL saturation control.
...
METADATA = {
    "name": "Saturate Effect",
    "description": "Saturate — HSL saturation control.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['saturate', 'hsl', 'color', 'vibrance', 'grade'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'saturation', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'vibrance', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'hue_shift', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'lightness', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'preserve_luminance', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float vibrance;  // Vibrance
uniform float hue_shift;  // Hue rotation
uniform float lightness;  // Lightness
uniform float preserve_luminance;  // Luma preserve
...
void main() {
    ...
    c.r *= 1.+hue_shift/10.0*0.2; c.g *= 1.+saturation/10.0*0.1; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['saturation', 'vibrance', 'hue_shift', 'lightness', 'preserve_luminance', 'mix']
_PARAM_DEFAULTS = {'saturation': 5.0, 'vibrance': 5.0, 'hue_shift': 0.0, 'lightness': 5.0, 'preserve_luminance': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class SaturatePlugin(EffectPlugin):
    """Saturate — HSL saturation control."""
    def __init__(self):
        super().__init__()

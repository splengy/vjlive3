"""
P7-VE53: Ben-Day dots — Roy Lichtenstein pop art halftone.
...
METADATA = {
    "name": "Ben Day Dots",
    "description": "Ben-Day dots — Roy Lichtenstein pop art halftone.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['pop-art', 'halftone', 'ben-day', 'dots', 'lichtenstein'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'dot_size', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'dot_spacing', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_reduce', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'angle', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'blend_mode', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float dot_size;  // Dot radius
uniform float dot_spacing;  // Dot pitch
uniform float color_reduce;  // Color levels
uniform float angle;  // Grid angle
uniform float blend_mode;  // Dot blend
...
void main() {
    ...
    c.r *= 1.+color_reduce/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['dot_size', 'dot_spacing', 'color_reduce', 'angle', 'blend_mode', 'mix']
_PARAM_DEFAULTS = {'dot_size': 5.0, 'dot_spacing': 5.0, 'color_reduce': 5.0, 'angle': 0.0, 'blend_mode': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class BenDayDotsPlugin(EffectPlugin):
    """Ben-Day dots — Roy Lichtenstein pop art halftone."""
    def __init__(self):
        super().__init__()

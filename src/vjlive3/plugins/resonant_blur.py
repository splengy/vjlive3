"""
P7-VE72: Resonant blur — frequency-modulated directional blur.
...
METADATA = {
    "name": "Resonant Blur",
    "description": "Resonant blur — frequency-modulated directional blur.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['blur', 'resonant', 'directional', 'frequency', 'vcv'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'blur_frequency', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'blur_amount', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'direction', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'resonance', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'bass_response', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float blur_frequency;  // Resonant frequency
uniform float blur_amount;  // Blur strength
uniform float direction;  // Blur direction
uniform float resonance;  // Resonance
uniform float bass_response;  // Bass drive
...
void main() {
    ...
    float d = (blur_amount/10.0); float fb = 0.0; float fr = 8.0+0.0*10.0;
    ...
    c.r *= 1.+blur_frequency/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['blur_frequency', 'blur_amount', 'direction', 'resonance', 'bass_response', 'mix']
_PARAM_DEFAULTS = {'blur_frequency': 5.0, 'blur_amount': 5.0, 'direction': 5.0, 'resonance': 5.0, 'bass_response': 5.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class ResonantBlurPlugin(EffectPlugin):
    """Resonant blur — frequency-modulated directional blur."""
    def __init__(self):
        super().__init__()

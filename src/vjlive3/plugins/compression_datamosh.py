"""
P5-DM10: Compression datamosh — DCT block artifacts, macroblocking, MPEG corruption.
Ported from VJlive-2/plugins/vdatamosh/compression_datamosh.py.
"""
...
METADATA = {
    "name": "Compression Datamosh",
    "description": "Compression datamosh — DCT block artifacts, macroblocking, MPEG corruption.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['compression', 'codec', 'artifact', 'datamosh', 'glitch'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'block_size', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'quality', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'motion_blur', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'artifact_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'macroblock_chaos', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_subsample', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'quantize_dc', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'quantize_ac', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'frame_skip', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'bit_starve', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'temporal_noise', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'error_concealment', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float block_size;  // DCT block size
uniform float quality;  // Compression level
uniform float motion_blur;  // Interframe smear
uniform float artifact_strength;  // Ringing
uniform float macroblock_chaos;  // Macroblock glitch
uniform float chroma_subsample;  // Chroma loss
uniform float quantize_dc;  // DC quantization
uniform float quantize_ac;  // AC quantization
uniform float frame_skip;  // Frame dropping
uniform float bit_starve;  // Bandwidth limit
uniform float temporal_noise;  // Interframe noise
uniform float error_concealment;  // Error hiding
...
void main() {
    ...
    float displacement = (block_size / 10.0);
    float feedback     = (error_concealment / 10.0);

    vec2 du = vec2(
        noise(uv * (8.0 + (quality / 10.0) * 10.0) + vec2(time * 0.5)) - 0.5,
        noise(uv * (8.0 + (quality / 10.0) * 10.0) + vec2(1.3, time * 0.4)) - 0.5
    ) * displacement * 0.1;
    ...
    float chroma = (chroma_subsample / 10.0);
    ...
    color.r *= 1.0 + 0.0;
    color.g *= 1.0 + 0.0;
    color.b *= 1.0 + artifact_strength/10.0*0.3;
...
_PARAM_NAMES = ['block_size', 'quality', 'motion_blur', 'artifact_strength', 'macroblock_chaos', 'chroma_subsample', 'quantize_dc', 'quantize_ac', 'frame_skip', 'bit_starve', 'temporal_noise', 'error_concealment']
_PARAM_DEFAULTS = {'block_size': 3.0, 'quality': 5.0, 'motion_blur': 3.0, 'artifact_strength': 5.0, 'macroblock_chaos': 2.0, 'chroma_subsample': 3.0, 'quantize_dc': 5.0, 'quantize_ac': 5.0, 'frame_skip': 2.0, 'bit_starve': 3.0, 'temporal_noise': 2.0, 'error_concealment': 5.0}
...
class CompressionDatamoshPlugin(EffectPlugin):

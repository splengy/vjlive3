"""
P5-DM28: Prism realm — refractive geometry, caustics, rainbow dispersion.
Ported from VJlive-2/plugins/vdatamosh/prism_realm_datamosh.py.
"""
...
METADATA = {
    "name": "Prism Realm Datamosh",
    "description": "Prism realm — refractive geometry, caustics, rainbow dispersion.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['prism', 'refraction', 'rainbow', 'datamosh', 'geometry'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'refraction', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'dispersion', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'facet_density', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'facet_shimmer', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'caustics_power', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'geometry_mix', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'glass_clarity', 'type': 'float', 'default': 7.0, 'min': 0.0, 'max': 10.0}, {'name': 'sacred_spin', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_refract', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'light_leak', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'prism_feedback', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'rainbow_pulse', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float refraction;  // Refraction amount
uniform float dispersion;  // Light dispersion
uniform float facet_density;  // Facet count
uniform float facet_shimmer;  // Shimmer
uniform float caustics_power;  // Caustics
uniform float geometry_mix;  // Geometry blend
uniform float glass_clarity;  // Clarity
uniform float sacred_spin;  // Spin speed
uniform float depth_refract;  // Depth refraction
uniform float light_leak;  // Light leak
uniform float prism_feedback;  // Feedback
uniform float rainbow_pulse;  // Rainbow pulse
...
void main() {
    ...
    float displacement = (refraction / 10.0);
    float feedback_v   = (prism_feedback / 10.0);
    float freq         = 8.0 + 5.0 * 10.0;
    float chroma       = (dispersion / 10.0);
    float vign_v       = 0.0;
    ...
    color.r *= 1.0 + rainbow_pulse/10.0*0.3;
    color.g *= 1.0 + sacred_spin/10.0*0.2;
    color.b *= 1.0 + prism_feedback/10.0*0.3;
    color = clamp(color, 0.0, 1.0);
    fragColor = mix(curr, vec4(color, curr.a), u_mix);
}
"""

_PARAM_NAMES = ['refraction', 'dispersion', 'facet_density', 'facet_shimmer', 'caustics_power', 'geometry_mix', 'glass_clarity', 'sacred_spin', 'depth_refract', 'light_leak', 'prism_feedback', 'rainbow_pulse']
_PARAM_DEFAULTS = {'refraction': 5.0, 'dispersion': 5.0, 'facet_density': 5.0, 'facet_shimmer': 5.0, 'caustics_power': 5.0, 'geometry_mix': 5.0, 'glass_clarity': 7.0, 'sacred_spin': 3.0, 'depth_refract': 5.0, 'light_leak': 3.0, 'prism_feedback': 3.0, 'rainbow_pulse': 5.0}
...
class PrismRealmDatamoshPlugin(EffectPlugin):

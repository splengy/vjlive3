"""
P5-DM32: Tunnel vision — claustrophobic radial zoom, heartbeat, vertigo.
Ported from VJlive-2/plugins/vdatamosh/tunnel_vision_datamosh.py.
"""
...
METADATA = {
    "name": "Tunnel Vision Datamosh",
    "description": "Tunnel vision — claustrophobic radial zoom, heartbeat, vertigo.",
    "version": "1.0.0",
    "plugin_type": "datamosh",
    "category": "datamosh",
    "tags": ['tunnel', 'vertigo', 'claustrophobic', 'datamosh', 'horror'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'focus_strength', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'peripheral_blur', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'tunnel_speed', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'warp_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'vignette_crush', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'spiral_twist', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'chroma_split', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'speed_lines', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'heartbeat', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'vertigo', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'flash_burn', 'type': 'float', 'default': 2.0, 'min': 0.0, 'max': 10.0}, {'name': 'contract', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float time;
uniform float u_mix;
uniform float focus_strength;  // Focus
uniform float peripheral_blur;  // Peripheral blur
uniform float tunnel_speed;  // Zoom speed
uniform float warp_depth;  // Warp depth
uniform float vignette_crush;  // Vignette
uniform float spiral_twist;  // Spiral
uniform float chroma_split;  // Chromatic
uniform float speed_lines;  // Speed lines
uniform float heartbeat;  // Heartbeat
uniform float vertigo;  // Vertigo
uniform float flash_burn;  // Flash burn
uniform float contract;  // Contraction
...
void main() {
    ...
    float displacement = (focus_strength / 10.0);
    float feedback_v   = (contract / 10.0);
    float freq         = 8.0 + (tunnel_speed / 10.0) * 10.0;
    float chroma       = (chroma_split / 10.0);
    float vign_v       = (vignette_crush / 10.0);
...
_PARAM_NAMES = ['focus_strength', 'peripheral_blur', 'tunnel_speed', 'warp_depth', 'vignette_crush', 'spiral_twist', 'chroma_split', 'speed_lines', 'heartbeat', 'vertigo', 'flash_burn', 'contract']
_PARAM_DEFAULTS = {'focus_strength': 5.0, 'peripheral_blur': 5.0, 'tunnel_speed': 5.0, 'warp_depth': 5.0, 'vignette_crush': 5.0, 'spiral_twist': 3.0, 'chroma_split': 2.0, 'speed_lines': 3.0, 'heartbeat': 3.0, 'vertigo': 5.0, 'flash_burn': 2.0, 'contract': 5.0}
...
class TunnelVisionDatamoshPlugin(EffectPlugin):

"""
P7-VE59: Rutt/Etra scanline synthesizer — depth-modulated scanlines.
...
METADATA = {
    "name": "Rutt Etra",
    "description": "Rutt/Etra scanline synthesizer — depth-modulated scanlines.",
    "version": "1.0.0",
    "plugin_type": "effect",
    "category": "vfx",
    "tags": ['rutt-etra', 'scanline', 'depth', 'analog', 'synth'],
    "priority": 1,
    "inputs": ["video_in"],
    "outputs": ["video_out"],
    "parameters": [{'name': 'scan_count', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'depth_amount', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'scan_width', 'type': 'float', 'default': 3.0, 'min': 0.0, 'max': 10.0}, {'name': 'color_from_depth', 'type': 'float', 'default': 5.0, 'min': 0.0, 'max': 10.0}, {'name': 'rotation', 'type': 'float', 'default': 0.0, 'min': 0.0, 'max': 10.0}, {'name': 'mix', 'type': 'float', 'default': 8.0, 'min': 0.0, 'max': 10.0}]
}
...
uniform float scan_count;  // Scanline count
uniform float depth_amount;  // Depth warp
uniform float scan_width;  // Line width
uniform float color_from_depth;  // Depth coloring
...
void main() {
    ...
    c.r *= 1.+color_from_depth/10.0*0.2; c.g *= 1.+0.0; c.b *= 1.+0.0;
    fragColor = mix(curr, vec4(clamp(c,0.,1.), curr.a), u_mix);
}
"""

_PARAM_NAMES = ['scan_count', 'depth_amount', 'scan_width', 'color_from_depth', 'rotation', 'mix']
_PARAM_DEFAULTS = {'scan_count': 5.0, 'depth_amount': 5.0, 'scan_width': 3.0, 'color_from_depth': 5.0, 'rotation': 0.0, 'mix': 8.0}

def _map(val,lo,hi): return lo+(max(0.,min(10.,float(val)))/10.)*(hi-lo)

class RuttEtraPlugin(EffectPlugin):
    """Rutt/Etra scanline synthesizer — depth-modulated scanlines."""
    def __init__(self):
        super().__init__()
